...
batWeap = weapon("Griffe","aaa",RANGE_MELEE,AREA_CIRCLE_1,65,100,0,emoji='<:griffe:884889931359596664>',use=STRENGTH)
...
carbunSkill = skill("Eclat Emeraude","aac",TYPE_DAMAGE,0,80,area=AREA_CIRCLE_2,accuracy=55,emoji='<:rafale:884889889445912577>',use=MAGIE,cooldown=4,description="Inflige des dégâts dans une large zone autour de la cible")
...
autoEff = effect("Explosé","aam",trigger=TRIGGER_END_OF_TURN,type=TYPE_INDIRECT_DAMAGE,power=99999,emoji=emojiMalus)
autoSkill = skill("Explosion","aan",TYPE_DAMAGE,0,100,AREA_MONO,emoji='<:defDamage:885899060488339456>',area=AREA_CIRCLE_1,effectOnSelf=autoEff,accuracy=200,setAoEDamage=True,description="Inflige des dégâts en zone autour de la Bombe Robot, puis s'auto détruit à la fin du tour*")
...
cutyBatSkill2Eff = effect("Renforcement Bis","batRenforceEff",stat=INTELLIGENCE,endurance=10,resistance=3,emoji=uniqueEmoji('<:egide:887743268337619005>'))
...
cutyBatSkill3Eff = effect("Echolocalisé","batEchoEff",CHARISMA,agility=-10,precision=-5,type=TYPE_MALUS)
...
cutyBatWeap = weapon("Onde sonore","batWeap",RANGE_DIST,AREA_CIRCLE_5,32,100,effectOnUse=incur[3],use=CHARISMA)
carbunRWeap = weapon("Griffe enflammé","rubyWeap",RANGE_MELEE,AREA_CIRCLE_1,52,100,use=MAGIE,emoji=batWeap.emoji)
carbunRSkill1 = skill("Pyrotechnie du Carbuncle","rubySkill",TYPE_DAMAGE,0,125,AREA_CIRCLE_2,cooldown=3,use=MAGIE,description="Inflige des dégâts à l'ennemi ciblé",emoji='<:hellFlame:1066788628845568030>')
...
carbObSkill2 = skill("Pelotonement Obsidienne",'carbObsSkill2',TYPE_BOOST,range=AREA_MONO,effects=carbObSkill2Eff,cooldown=3,replay=True,description="Augmente l'aggression du Carbuncle Obsidienne et ses chances d'esquiver des attaques en lui permettant de rejouer son tour",emoji=carbObSkill2Eff.emoji[0][0])
...
killerWailSkill = skill("Rayon sonique","killerWail5.1SumSkill",TYPE_DAMAGE,0,70,AREA_CIRCLE_5,area=AREA_LINE_6,effectOnSelf=autoEff,use=MAGIE,setAoEDamage=True,emoji='<:kwShot:933510363398418432>')
...
lightButterflyWeap = weapon("Lueur Volatile","lightButterflyWeap",RANGE_DIST,AREA_CIRCLE_5,power=65,accuracy=100,use=CHARISMA,type=TYPE_HEAL,target=ALLIES,ignoreAutoVerif=True)
lightButterflySkill1 = skill("Eclats Lumineux","lightButterflySkill1",TYPE_HEAL,power=75,range=AREA_MONO,area=AREA_CIRCLE_3)
lightButterflySkill2Eff = effect("Bénédiction Lumineuse","lightButterflySkill2Eff",CHARISMA,type=TYPE_INDIRECT_HEAL,power=50,turnInit=3,trigger=TRIGGER_START_OF_TURN,stackable=True)
...
skeletonWeap = weapon("Epée nécromantique","skeletonWeap",RANGE_MELEE,AREA_CIRCLE_1,batWeap.power,batWeap.accuracy,0,strength=10,magie=10,use=MAGIE,emoji=ironSword.emoji,ignoreAutoVerif=True)
