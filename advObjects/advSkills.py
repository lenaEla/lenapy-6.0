...
soupledown = skill("Choc Ténébreux","zz",TYPE_DAMAGE,500,150,range = AREA_INLINE_3,condition=[EXCLUSIVE,ASPIRATION,POIDS_PLUME],area = AREA_CIRCLE_2,sussess=150,tpCac=True,ultimate=True,cooldown=5,emoji='<:lunadown:932444767504203847>',use=STRENGTH)
...
draw25 = skill("Stop attacking or draw 25","zq",TYPE_MALUS,300,25,emoji="<:draw25:869982277701103616>",use=None,effects="lq",cooldown = 3,area=AREA_ALL_ENEMIES,range=AREA_MONO,ultimate=True,condition=[EXCLUSIVE,ASPIRATION,SORCELER],message="{0} utilise sa carte joker !")
...
explosionCast = skill("Explosion","zf",TYPE_DAMAGE,1000,0,ultimate=True,cooldown=7,area=AREA_CIRCLE_2,sussess=0,shareCooldown=True,effectOnSelf=castExplo,use=MAGIE,emoji='<a:explosion:882627170944573471>',message="{0} rassemble son mana...")
...
descart = skill("Nous pensons donc nous somme","yu",TYPE_BOOST,250,range=AREA_MONO,area=AREA_CIRCLE_2,emoji="<:descartes:885240392860188672>",effects='mi',cooldown=5,use=None,message="{0} a trouvé le sens de la vie !")
...
burst = skill("Bombe ballon","ys",TYPE_DAMAGE,0,75,area=AREA_CIRCLE_1,sussess=70,emoji='<:burstBomb:887328853683474444>',use=HARMONIE,cooldown=3,setAoEDamage=True)
...
stingray = skill("Pigmalance","wc",TYPE_DAMAGE,500,135,AREA_CIRCLE_7,ultimate=True,emoji='<:stingray:899243721378390036>',cooldown=5,area=AREA_LINE_6,sussess=100,effectOnSelf=pigmaCast,damageOnArmor=0.7,url="https://cdn.discordapp.com/attachments/935769576808013837/935781565072547850/20220126_071411.gif")
...
convert = skill("Convertion","vm",TYPE_ARMOR,350,range=AREA_DONUT_5,effects="nk",cooldown=3,emoji='<:convertion:900311843938115614>')
vampirisme = skill("Vampirisme","vl",TYPE_INDIRECT_HEAL,350,range=AREA_DONUT_5,effects="no",cooldown=3,emoji='<:vampire:900312789686571018>',group=SKILL_GROUP_DEMON,hpCost=10,effPowerPurcent=150)
vampirisme2 = skill("Vampirisme Avancée","spw",TYPE_INDIRECT_HEAL,500,effects="no",cooldown=5,emoji='<:vampire:900312789686571018>',group=SKILL_GROUP_DEMON,hpCost=20,effPowerPurcent=75,area=AREA_CIRCLE_3,range=AREA_MONO)
...
suppr = skill("Suppression","vh",TYPE_DAMAGE,650,115,emoji='<a:sup:925199175681970216>',cooldown=5,use=MAGIE,damageOnArmor=3,sussess=80)
...
contrainte = skill("Contrainte","uz",TYPE_MALUS,500,range=AREA_CIRCLE_6,effects=[incur[3],"oc"],cooldown=3,use=INTELLIGENCE,emoji='<:contrainte:963393945704992829>')
trouble = skill("Trouble","uy",TYPE_MALUS,500,range=AREA_CIRCLE_6,effects=[incur[3],"od"],use=CHARISMA,emoji='<:trouble:963394783596933182>',cooldown=3)
...
blackHole2 = skill("Trou noir Avancé","uj",TYPE_BOOST,0,use=None,effects="on",effectOnSelf=blackHoleEff2,emoji='<:blackHole2:906195979332640828>',cooldown=7,ultimate=True,description="Redirige **25%** des dégâts directs reçus par vos alliés proches vers vous tout en réduisant de **25%** les dégâts indirects qu'ils recoivent et en diminuant leur agression, et réduit les dégâts que vous recevez de **25%** tout en augmentant votre agression pendant 3 tours",area=AREA_DONUT_2,range=AREA_MONO)
...
hinaUlt = skill("Déluge de plume","tj",TYPE_DAMAGE,0,40,sussess=50,area=AREA_CONE_2,repetition=5,cooldown=7,emoji='<:featherStorm:909932475457884191>')
...
mageUltMono.power,mageUltMono.area,mageUltMono.sussess = 180,AREA_MONO,160
mageUltZone = copy.deepcopy(mageUlt)
mageUltZone.power,mageUltZone.area,mageUltZone.sussess = 120,AREA_CONE_3,80
...
geoConLaunch = skill("Géo-Controle","geoControleFinal",TYPE_BOOST,1000,range=AREA_MONO,area=AREA_CIRCLE_3,effects=geoConBoost,cooldown=7,use=INTELLIGENCE,ultimate=True,emoji='<:geoCont:918422710781083668>')
...
supprZone = skill("Dispersion","zzx",TYPE_DAMAGE,650,100,emoji='<:principio:919529034663223376>',cooldown=5,use=MAGIE,damageOnArmor=3,sussess=60,area=AREA_CIRCLE_1)
...
darkSweetHeat = skill("Flammes infernales","zyp",TYPE_DAMAGE,500,75,range=AREA_DIST_5,cooldown=7,effectOnSelf=sweetHeatEff,use=MAGIE,area=AREA_CONE_2,group=SKILL_GROUP_DEMON,condition=[EXCLUSIVE,ELEMENT,ELEMENT_FIRE],emoji='<:infFlames:931716686745305149>')
...
lohicaUltLauch = skill("Brûme empoisonée","suz",TYPE_INDIRECT_DAMAGE,750,area=AREA_CIRCLE_2,use=MAGIE,tpCac=True,effects="me",cooldown=7,ultimate=True,emoji='<:brume:931908895083991100>',areaOnSelf=True,description="Téléporte à côté de la cible et empoisonne les alentours",effPowerPurcent=200)
...
autoBombRush = skill("Lance-Bombe Robot",'sut',TYPE_SUMMON,750,0,AREA_CIRCLE_4,[EXCLUSIVE,ASPIRATION,ASPI_NEUTRAL],True,nbSummon=3,emoji='<:brAuto:933508393036029992>',invocation="Bombe Robot",cooldown=7,description="Invoque 3 Bombes Robots\nSeule la première Bombe Robot est soumise à la limitation d'invocation par équipe",shareCooldown=True)
killerWailUltimate = skill("Haut Perceur 5.1",'sus',TYPE_SUMMON,750,condition=[EXCLUSIVE,ASPIRATION,ASPI_NEUTRAL],nbSummon=5,ultimate=True,cooldown=10,use=MAGIE,invocation='Haut-Perceur 5.1',emoji='<:killerWail:933516496196497439>',description='Invoque 5 Haut-Perceur 5.1.\nSeul le premier est soumis à la limitation d\'invocation par équipe',shareCooldown=True)
...
darkBoom = skill("Explosion Sombre",'darkBoom',TYPE_DAMAGE,500,int(explosion.power*0.35),cooldown=explosion.cooldown,percing=0,area=AREA_CIRCLE_2,effectOnSelf=explosion.effectOnSelf,sussess=explosion.sussess,setAoEDamage=True,repetition=2,ultimate=True,use=MAGIE,emoji='<a:db2:933676696899551273>',hpCost=30,group=SKILL_GROUP_DEMON,description="Inflige des dégâts dans une grande zone à deux reprises",damageOnArmor=0.8)
...
foulleeEff = effect("Foulée légère",'ppUniquePassifEff',turnInit=-1,unclearable=True,dodge=20,description="Augmente de **20%** votre probabilité d'esquiver des attaques")
...
heartStone = skill("Coeur de pierre","stj",TYPE_ARMOR,750,cooldown=3,emoji='<:earthStonr:941681433863421952>',effects=[impactShield, heartStoneSecEff],condition=[EXCLUSIVE,ASPIRATION,PROTECTEUR],description="Confère une petite armure à l'allié ciblé et réduit ses dégâts reçus\nL'armure dure plus longtemps que la réduction de dégâts")
erodStrike = skill("Frappe érosive","sti",TYPE_DAMAGE,500,105,AREA_CIRCLE_3,cooldown=5,erosion=65,emoji='<:frappeEro:941677350872047640>')
...
holmgang = skill("Holmgang","sst",TYPE_BOOST,500,range=AREA_MONO,effPowerPurcent=200,emoji='<:complicated:945054889573240883>',use=None,effects=holmgangEff,cooldown=7,effectAroundCaster=[TYPE_MALUS,AREA_DONUT_2,chained],description="Empêche vos PV de tomber en dessous de 1 pendant 1 tour")
...
galvanisation = skill("Galvanisation",'ssr',TYPE_DAMAGE,750,emoji='<:galvaPheonix:946042576413290506>',area=AREA_CIRCLE_1,power=135,use=MAGIE,effectAroundCaster=[TYPE_INDIRECT_HEAL,AREA_CIRCLE_2,phoenixFlight],cooldown=7,description="Inflige des dégâts aux ennemis ciblés et donne un effet régénérant aux alliés autour de vous\n\n{0} __{1}__ :\nSoigne le porteur avec une puissance de **{2}** en utilisant la statistique de magie pendant 3 tours".format(phoenixFlight.emoji[0][0],phoenixFlight.name,phoenixFlight.power))
...
windBurn = effect("Morsure du vent","windBurn",AGILITY,power=35,turnInit=3,lvl=3,area=AREA_DONUT_1,emoji='<:windBite:961185646993637476>',trigger=TRIGGER_END_OF_TURN,description="Inflige des dégâts magiques aux ennemis aux corps à corps du porteur à la fin du tour de celui-ci",type=TYPE_INDIRECT_DAMAGE)
...
coroWind = effect("Mordax Ventus","coroWind",MAGIE,power=40,turnInit=3,stackable=True,lvl=3,trigger=TRIGGER_END_OF_TURN,type=TYPE_INDIRECT_DAMAGE,emoji='<:corWind:968321700922028062>',description="Un vent empoisonné qui fait des dégâts à la fin du tour du porteur.\nPossède une légère odeur de sang")

cwFocus = skill("Incarnatus Puff","sqw",TYPE_INDIRECT_DAMAGE,500,cooldown=focal.cooldown,effects=coroWind,effPowerPurcent=300,effectOnSelf=coroWind,description="Donne un effet {effIcon} __{effName}__ trois fois plus puissant à l'ennemi ciblé, mais en subissez un également",emoji='<:IncPuf:968327189508542484>')
...
cwUlt = skill("Ultima Exitium","squ",TYPE_INDIRECT_DAMAGE,750,ultimate=True,cooldown=7,effects=coroWind,power=25,description="Inflige l'effet {effIcon} __{effName}__ à la cible, puis inflige immédiatement des dégâts indirects en fonction du nombre d'effets de Dégâts Indirects ainsi que leur durée restante présents sur la cible\nLes effets autres que {effIcon} __{effName}__ augmentent un peu plus la puissance de l'attaque mais sont consommés par cette dernière")
sanguisGladio = skill("Sanguis Gladio","sqt",TYPE_DAMAGE,500,80,cooldown=5,effects=coroWind,effPowerPurcent=80,use=MAGIE,emoji='<:sgladio:968478994724950056>')
sanguisGladio2 = skill("Sanguis Gladio Avancée","sqs",TYPE_DAMAGE,750,100,cooldown=7,area=AREA_LINE_2,effects=coroWind,effPowerPurcent=65,use=MAGIE,emoji='<:sgladio2:968479144293855252>')
...
solarEruption = skill("Eruption Solaire","spy",TYPE_DAMAGE,500,100,area=AREA_ARC_1,cooldown=5,condition=[EXCLUSIVE,ELEMENT,ELEMENT_SPACE],use=MAGIE)
...
plumRemEff = effect("Plumes Rémanantes","plumRemEff",STRENGTH,power=75,area=AREA_CIRCLE_1,turnInit=6,type=TYPE_INDIRECT_DAMAGE,emoji="<:plumeRem:989543669839319102>",stackable=True)
...
sillage = skill("Sillage",getAutoId(cycloneEcarlate.id,True),TYPE_DAMAGE,500,sillageEff.power+10,use=MAGIE,area=AREA_CIRCLE_1,range=AREA_CIRCLE_4,effects=sillageEff,cooldown=7,emoji=sillageEff.emoji[0][0],description="Inflige des dégâts à l'ennemi ciblé et lui inflige un effet de poison qui infligera des dégâts en zone durant les deux prochains tours",condition=[EXCLUSIVE,ELEMENT,ELEMENT_AIR])
...
demonArmor2 = skill("Armure Démoniaque Avancée",getAutoId(demonArmor.id,True),TYPE_ARMOR,750,effects=demonArmorEff,cooldown=5,group=SKILL_GROUP_DEMON,hpCost=15,area=AREA_CIRCLE_3,range=AREA_MONO,emoji='<:dmonArmor:1006455374464946226>')
...
lohicaBrouillad = skill("Déployable - Brouillard Empoisonné", getAutoId(bigBubbler.id,True), TYPE_DEPL, 500, emoji=lohicaDepl.icon[0],cooldown=7, depl=lohicaDepl, use=MAGIE,range=AREA_CIRCLE_3)
...
antConst = skill("Anticonstitution",getAutoId(constBen.id,True),TYPE_DAMAGE,750,power=100,range=AREA_CIRCLE_3,effects=antConstEff,cooldown=7,condition=[EXCLUSIVE,ASPIRATION,TETE_BRULE],emoji='<:emoji_4:1028695463651717200>',erosion=50,description="Inflige des dégâts un ennemi et réduit temporairement ses PV Maximums")
