...
EXCLUSIVE = 0
WEAPON, ASPIRATION, ELEMENT, SECELEM = 0,1,2,3

soupledown = skill("Choc Ténébreux","zz",TYPE_DAMAGE,500,150,range = AREA_INLINE_3,condition=[EXCLUSIVE,ASPIRATION,POIDS_PLUME],area = AREA_CIRCLE_2,sussess=150,tpCac=True,ultimate=True,cooldown=6,emoji='<:lunadown:932444767504203847>',use=STRENGTH)
...
coffeeSkill = skill("SupréCafé","zx",TYPE_BOOST,500,effect=["lb"],use=CHARISMA,area=AREA_ALL_ALLIES,emoji='<:coffee:867538582846963753>',cooldown=5,message="{0} prend le café avec ses alliés :")
theSkill = skill("LiberThé","zw",TYPE_BOOST,500,effect=["lc"],use=CHARISMA,area=AREA_ALL_ALLIES,emoji='<:the:867538602644602931>',cooldown=5,message="{0} prend le thé avec ses alliés :")
...
zelian = skill("Chronoshift","zt",TYPE_INDIRECT_HEAL,500,cooldown=7,effect="lj",emoji='<:chronoshift:867872479719456799>',condition=[EXCLUSIVE,ELEMENT,ELEMENT_TIME])
courage = skill("Motivation","zs",TYPE_BOOST,500,emoji ='<:charge:866832551112081419>',area=AREA_CIRCLE_2,use=CHARISMA,effect="lk",cooldown=5,range=AREA_MONO)
nostalgia = skill("Nostalgie","zr",TYPE_MALUS,500,emoji='<:nostalgia:867162802783649802>',effect="lm",cooldown=5,use=INTELLIGENCE)
draw25 = skill("Stop attacking or draw 25","zq",TYPE_MALUS,300,25,emoji="<:draw25:869982277701103616>",use=None,effect="lq",cooldown = 3,area=AREA_ALL_ENEMIES,range=AREA_MONO,ultimate=True,condition=[EXCLUSIVE,ASPIRATION,SORCELER],message="{0} utilise sa carte joker !")
siropMenthe = skill("Menthalité","zp",TYPE_BOOST,500,effect=["lu"],use=CHARISMA,area=AREA_ALL_ALLIES,emoji='<:menthe:867538622797054042>',cooldown=5,message="{0} prend un sirop de menthe avec ses alliés :")
...
multiMissiles = skill("Multi-Missiles","zb",TYPE_INDIRECT_DAMAGE,750,range=AREA_MONO,ultimate=True,emoji='<:tentamissile:884757344397951026>',effect="mf",cooldown=5,area=AREA_RANDOMENNEMI_5,url='https://cdn.discordapp.com/attachments/935769576808013837/935781565663948850/20220126_071157.gif')
monoMissiles = skill("Mono-Missiles","za",TYPE_INDIRECT_DAMAGE,250,range=AREA_CIRCLE_7,emoji='<:monomissile:884757360193708052>',effect="mf",cooldown=5)
...
descart = skill("Nous pensons donc nous somme","yu",TYPE_BOOST,250,range=AREA_MONO,area=AREA_CIRCLE_2,emoji="<:descartes:885240392860188672>",effect='mi',cooldown=5,use=None,message="{0} a trouvé le sens de la vie !")
...
burst = skill("Bombe ballon","ys",TYPE_DAMAGE,0,65,area=AREA_CIRCLE_1,sussess=70,emoji='<:burstBomb:887328853683474444>',use=HARMONIE,cooldown=3,setAoEDamage=True)
...
linx = skill("Œuil de Linx","yl",TYPE_BOOST,500,range=AREA_MONO,area=AREA_CIRCLE_2,emoji='<:noeuil:887743235131322398>',effect="mm",cooldown=5)
...
secondSun = skill("Le second Soleil","yh",TYPE_MALUS,350,0,AREA_MONO,area=AREA_ALL_ENEMIES,cooldown=5,effect="mq",emoji='<:iwanttosleeppls:893241882484817920>',use=CHARISMA,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT])
...
innerdarkness = skill("Ténèbres intérieurs","ya",TYPE_INDIRECT_DAMAGE,500,0,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS],ultimate=True,emoji='<:emoji_48:893471268957990982>',cooldown=5,effect="ms",use=MAGIE,group=SKILL_GROUP_DEMON,hpCost=25)
divineLight = skill("Lumière divine",'xz',TYPE_INDIRECT_HEAL,500,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT],ultimate=True,emoji='<:emoji_49:893471282815963156>',cooldown=5,effect=["mu","mv"],use=CHARISMA,area=AREA_CIRCLE_1,group=SKILL_GROUP_HOLY,maxHpCost=15)
...
bleedingArrow = skill("Flèche Hémorragique","ws",TYPE_DAMAGE,350,60,AREA_DIST_5,effect='mx',emoji='<:bleedingarrow:894544820071178292>',cooldown=3)
bleedingDague = skill("Dague Hémorragique","wr",TYPE_DAMAGE,350,60,AREA_CIRCLE_2,effect='mx',emoji='<:bleedingdague:894552391444234242>',cooldown=3)
...
flameche = skill("Flamèche","wo",TYPE_DAMAGE,100,60,area=AREA_CONE_2,cooldown=3,use=MAGIE,condition=[EXCLUSIVE,ELEMENT,ELEMENT_FIRE],emoji='<:flame1:897811975675969556>')
flame = skill("Flamme","wn",TYPE_DAMAGE,250,85,area=AREA_CONE_2,use=MAGIE,condition=[EXCLUSIVE,ELEMENT,ELEMENT_FIRE],cooldown=5,emoji='<:flame2:897812264185376798>')
pyro = skill("Pyrotechnie","wm",TYPE_DAMAGE,500,110,area=AREA_CONE_2,use=MAGIE,condition=[EXCLUSIVE,ELEMENT,ELEMENT_FIRE],cooldown=7,emoji='<:flame3:897812061139140651>')
...
derobade = skill("Dérobade","vv",TYPE_BOOST,350,0,AREA_DONUT_3,cooldown=3,effectOnSelf="nd",effect="nc",emoji='<:derobade:899788297868558337>',use=INTELLIGENCE)
...
bleedingTrap = skill("Piège de lacération","vp",TYPE_INDIRECT_DAMAGE,500,effect="mx",cooldown=5,area=AREA_CIRCLE_1,use=STRENGTH,message="{0} place et déclanche un {1} autour de {2} :",emoji='<:lacerTrap:900076484230774807>',effPowerPurcent=55)
...
onde = skill("Onde","vf",TYPE_ARMOR,500,effect="nv",cooldown=3,area=AREA_CIRCLE_1,emoji='<:onde:902526595842072616>')
...
demolish = skill("Démolition","va",TYPE_DAMAGE,750,180,AREA_CIRCLE_2,ultimate=True,cooldown=7,effect=incur[5],damageOnArmor=1.2,emoji='<:destruc:905051623108280330>',url="https://media.discordapp.net/attachments/1006418796329848954/1007574358551699466/20220812_105440.gif")
...
concen = skill("Concentration","um",TYPE_BOOST,price=350,effect="oj",range=AREA_MONO,area=AREA_DONUT_2,cooldown=3,use=None,emoji='<:concen:921762263814262804>')
...
blackHoleEff2.power, blackHoleEff2.aggro, blackHoleEff2.turnInit = 25, 35, 3
...
fireShot = skill("Tir Feu","ue",TYPE_DAMAGE,350,70,area=AREA_CONE_2,cooldown=3,condition=[EXCLUSIVE,ELEMENT,ELEMENT_FIRE],range=AREA_DIST_5,emoji='<:fireShot:906219594639876116>')
waterShot = skill("Tir Eau","ud",TYPE_DAMAGE,350,100,cooldown=3,condition=[EXCLUSIVE,ELEMENT,ELEMENT_WATER],range=AREA_DIST_5,emoji='<:waterShot:906219607856119868>')
airStrike = skill("Frappe Air","uc",TYPE_DAMAGE,350,70,area=AREA_CONE_2,cooldown=3,condition=[EXCLUSIVE,ELEMENT,ELEMENT_AIR],range=AREA_CIRCLE_2,emoji='<:airStrike:906219547873386526>')
earthStrike = skill("Frappe Terre","ub",TYPE_DAMAGE,350,100,cooldown=3,condition=[EXCLUSIVE,ELEMENT,ELEMENT_EARTH],range=AREA_CIRCLE_2,emoji='<:earthStrike:906219563832709142>')
...
timeSp = skill("Rembobinage","tu",TYPE_INDIRECT_HEAL,500,use=CHARISMA,range=AREA_MONO,area=AREA_CIRCLE_5,effect=timeSpEff,condition=[EXCLUSIVE,ELEMENT,ELEMENT_TIME],cooldown=7,ultimate=True,emoji='<:rlb:907687694476378112>')
...
tripleMissilesLaunch = skill("Missiles téléguidés","tripleMissiles",TYPE_DAMAGE,750,120,ultimate=True,repetition=3,damageOnArmor=1.2,percing=0,cooldown=10,emoji='<:missiles:909727253414445057>',condition=[EXCLUSIVE,ASPIRATION,OBSERVATEUR])
...
hinaUlt = skill("Déluge de plume","tj",TYPE_DAMAGE,0,35,sussess=40,area=AREA_CONE_2,repetition=5,cooldown=7,emoji='<:featherStorm:909932475457884191>')
...
mageUlt = skill("Éminence","tg",TYPE_DAMAGE,1000,emoji='<:emanence:911397114850975795>',power=120,condition=[EXCLUSIVE,ASPIRATION,MAGE],ultimate=True,cooldown=5,area=AREA_DONUT_2,use=MAGIE,description="Une compétence dont la puissance et la zone d'effet dépend de l'élément")
...
firelame = skill("Ignis ferrum","sr",TYPE_DAMAGE,500,38,range=AREA_DIST_5,condition=[EXCLUSIVE,ELEMENT,ELEMENT_FIRE],cooldown=7,area=AREA_ARC_1,repetition=3,emoji='<:fireLame:916130324197543986>',damageOnArmor=1.1)
airlame = skill("Aer ferrum","sq",TYPE_DAMAGE,500,38,range=AREA_CIRCLE_3,condition=[EXCLUSIVE,ELEMENT,ELEMENT_AIR],cooldown=7,area=AREA_ARC_1,repetition=3,emoji='<:airLame:916130609095655454>',damageOnArmor=1.1)
...
shadowLame = skill("Umbra ferrum","sn",TYPE_INDIRECT_DAMAGE,500,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS],effect=[shadowLameEff,shadowLameEff,shadowLameEff],cooldown=7,emoji='<:darkLame:916130473162453033>')
...
lightLame = skill("Lux ferrum","sm",TYPE_BOOST,500,effect=lightLameEff,emoji='<:lightLame:916131830149816340>',cooldown=7,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT],use=CHARISMA)
...
astralLame = skill("Stella ferrum","sl",TYPE_BOOST,500,range=AREA_MONO,effect=astralLameEff,emoji='<:astralLame:916131872352911400>',cooldown=7,condition=[EXCLUSIVE,ELEMENT,ELEMENT_SPACE],use=ENDURANCE)
...
timeLame = skill("Tempus ferrum","sk",TYPE_BOOST,500,effect=timeLameEff,emoji='<:timeLame:916131853499527208> ',cooldown=7,condition=[EXCLUSIVE,ELEMENT,ELEMENT_TIME],use=CHARISMA)
...
cosmicPower = skill("Pouvoir cosmique","zzt",TYPE_BOOST,1000,range=AREA_MONO,area=AREA_DONUT_2,use=HARMONIE,cooldown=7,emoji='<:cosmic:919866054992334848>',condition=[EXCLUSIVE,ELEMENT,ELEMENT_SPACE],effect=cosmicEff,effectOnSelf=cosmicEffSelf)
...
inkRes = skill("Pied au sec","zzr",TYPE_BOOST,500,effect=inkResEff,cooldown=5,area=AREA_CIRCLE_1,use=INTELLIGENCE,emoji='<:inkResR:921486021265354814>')
inkRes2 = skill("Inconscient collectif","zzq",TYPE_BOOST,500,range=AREA_MONO,area=AREA_CIRCLE_2,effect=inkResEff2,cooldown=5,use=ENDURANCE,emoji='<:incCol:921545816940875817>')
...
foyer = skill("Foyer","zyr",TYPE_HEAL,500,35,range=AREA_MONO,area=AREA_CIRCLE_2,cooldown=5,use=CHARISMA,condition=[EXCLUSIVE,SECELEM,ELEMENT_FIRE],emoji="<:fireplace:931666078092902471>")
...
sweetHeat = skill("Flammes intérieurs","zyq",TYPE_DAMAGE,500,85,range=AREA_CIRCLE_3,cooldown=7,effectOnSelf=sweetHeatEff,condition=[EXCLUSIVE,SECELEM,ELEMENT_FIRE],emoji='<:fireStrike:931715107002662912>')
darkSweetHeat = skill("Flammes infernales","zyp",TYPE_DAMAGE,500,75,range=AREA_DIST_5,cooldown=7,effectOnSelf=sweetHeatEff,use=MAGIE,area=AREA_CONE_2,group=SKILL_GROUP_DEMON,condition=[EXCLUSIVE,SECELEM,ELEMENT_FIRE],emoji='<:infFlames:931716686745305149>')
...
shell = skill("Coquille",'zyo',TYPE_ARMOR,500,range=AREA_CIRCLE_7,use=HARMONIE,effect=nacre,effectOnSelf=nacre,cooldown=5,condition=[EXCLUSIVE,SECELEM,ELEMENT_WATER],emoji='<:shell:931714079154921472>')
nacreHit = skill("Frappe nacrée",'syn',TYPE_DAMAGE,500,80,range=AREA_CIRCLE_3,effectOnSelf=nacre,cooldown=5,use=HARMONIE,condition=[EXCLUSIVE,SECELEM,ELEMENT_WATER])
...
fisure = skill("Fissure",'svg',TYPE_DAMAGE,500,80,condition=[EXCLUSIVE,SECELEM,ELEMENT_EARTH],cooldown=5,damageOnArmor=3,emoji='<:break:931703557420367873>')
seisme = skill("Séisme",'svf',TYPE_DAMAGE,500,70,range=AREA_MONO,area=AREA_CIRCLE_3,condition=[EXCLUSIVE,SECELEM,ELEMENT_EARTH],cooldown=5,damageOnArmor=3,emoji='<:seisme:931703529561800744>')
...
aliceDanceFinal = skill("Chorégraphie Dynamique","corDynFinal",TYPE_BOOST,500,range=AREA_MONO,area=AREA_DONUT_4,effect=aliceDanceEff,emoji='<:dyna:932618114892439613>',use=CHARISMA,cooldown=5,message="{0} se donne à fond !",description="Se met à danser pendant 3 tours, durant lesquels les alliés à portée voient leur statistiques offensives augmenter",effPowerPurcent=125)
...
aurore = skill('Aurore','sux',TYPE_BOOST,500,effect=auroreEff,use=CHARISMA,cooldown=7,area=AREA_CIRCLE_1,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT],emoji='<:aurore:934675235268087858>')
...
crep = skill('Crépuscule','suw',TYPE_MALUS,500,effect=crepEff,use=INTELLIGENCE,cooldown=7,area=AREA_CIRCLE_1,condition=[EXCLUSIVE,SECELEM,ELEMENT_DARKNESS],emoji='<:crepuscule:934675249541292033>')
...
killerWailUltimate = skill("Haut Perceur 5.1",'sus',TYPE_SUMMON,750,condition=[EXCLUSIVE,ASPIRATION,ASPI_NEUTRAL],ultimate=True,cooldown=10,use=MAGIE,invocation='Haut-Perceur 5.1',emoji='<:killerWail:933516496196497439>',description='Invoque 5 Haut-Perceur 5.1.\nSeul le premier est soumis à la limitation d\'invocation par équipe',shareCooldown=True)
...
mageSkill = skill("Glace",'sun',TYPE_DAMAGE,500,65,condition=[EXCLUSIVE,'aspiration',MAGE],area=AREA_CIRCLE_1,setAoEDamage=True,cooldown=3,use=MAGIE,emoji='<:ice:941494417926270987>')
...
extraMedica = skill("Extra Medica",'suk',TYPE_HEAL,750,int(infraMedica.power*0.8),AREA_MONO,area=AREA_CIRCLE_2,use=CHARISMA,effect=infraMedica.effect,cooldown=infraMedica.cooldown+2,group=SKILL_GROUP_HOLY,condition=[EXCLUSIVE,'aspiration',ALTRUISTE],description="Soigne les alliés proches et leur donne l'effet __Extra Médica__ pendant 3 tours",maxHpCost=10,emoji=infraMedica.emoji)
...
pneuma = skill("Pneuma",'suf',TYPE_DAMAGE,750,125,condition=[EXCLUSIVE,ASPIRATION,PREVOYANT],ultimate=True,emoji='<:pneuma:936515231562219581>',cooldown=7,area=AREA_LINE_3,sussess=110,use=INTELLIGENCE,useActionStats=ACT_SHIELD,effectAroundCaster=[TYPE_ARMOR,AREA_CIRCLE_2,pneumaArmor])
...
faucheCroix = skill("Gibet",'sts',TYPE_DAMAGE,power=80,range=AREA_CIRCLE_2,effectOnSelf=faucheCroixEff,cooldown=1,area=AREA_ARC_2,emoji='<:gibet:1007583449273614378>')
...
reaperEff = effect("Dessins de la Camarade", "that'sALongName", power=30, turnInit=4, type=TYPE_MALUS, emoji="<:longName:938167649106538556>",description="Augmente de **10%** les dégâts infligés par l'entité à l'origine de cet effect sur le porteur.\nSi le porteur est vaincu en ayant toujours l'effet sur lui (que ce soit de la main de l'initiateur de l'effet ou d'un autre), l'initiateur de l'effet se soigne de **{0}%** de ses PV maximums et sa Jauge d'Âme augmente de 5 pts supplémentaires")
deathShadow = skill("Ombre de la Mort","str",TYPE_DAMAGE,750,90,range=AREA_CIRCLE_2,emoji='<:deathShadow:937370374788755516>',effBeforePow=True,cooldown=7,effect=reaperEff,description="Inflige des dégâts et donne à la cible augmentant vos futurs dégâts à son encontre pendant plusieurs tours")
...
heartStone = skill("Coeur de pierre","stj",TYPE_ARMOR,750,cooldown=3,emoji='<:earthStonr:941681433863421952>',effect=[impactShield, heartStoneSecEff],condition=[EXCLUSIVE,ASPIRATION,PROTECTEUR],description="Confère une petite armure à l'allié ciblé et réduit ses dégâts reçus\nL'armure dure plus longtemps que la réduction de dégâts")
...
ice2 = skill("Glace II","stf",TYPE_INDIRECT_DAMAGE,500,use=MAGIE,cooldown=3,emoji='<:ice2:941494399337136208>',condition=[EXCLUSIVE,ASPIRATION,SORCELER],effect=ice2Eff)
...
renf2 = skill("Renforcement Avancé","std",TYPE_BOOST,750,condition=[EXCLUSIVE,ASPIRATION,INOVATEUR],effect=[renf2Eff2,renf2Eff1],cooldown=7,emoji='<:renfAv:989529323063107605>')
...
nova = skill("Nova","ssy",TYPE_DAMAGE,500,90,use=INTELLIGENCE,useActionStats=ACT_SHIELD,hpCost=15,cooldown=5,group=SKILL_GROUP_DEMON)
undeadEff2 = effect("Mort en sursie","undeadEff2",turnInit=3,percing=15,aggro=-15,description="Tant que cet effet est actif, le porteur ne peut pas être vaincu\nCet effet est retiré si le porteur est soigné d'un montant égalant **50%** de ses PV max au moment de la pose de l'effet durant sa durée\nSi l'effet atteint la fin de sa durée, le porteur est automatiquement vaincu\nAugmente les soins reçus et accorde un bonus de vol de vie de **{0}** tant que l'effet est actif",emoji='<a:undead:953259661568663602>',silent=False,power=50)
...
undead = skill("Mort Vivant","ssx",TYPE_PASSIVE,500,effectOnSelf=undeadEff,emoji='<:undead:944989590836637736>',group=SKILL_GROUP_DEMON,quickDesc="Vous permet de transcender une fois la mort tout en vous empêchant de mourir durant 3 tours durant lesquel vous devez être soigné")
...
holmgang = skill("Holmgang","sst",TYPE_BOOST,500,range=AREA_MONO,effPowerPurcent=200,emoji='<:complicated:945054889573240883>',use=None,effect=holmgangEff,cooldown=7,effectAroundCaster=[TYPE_MALUS,AREA_DONUT_2,chained],description="Empêche vos PV de tomber en dessous de 1 pendant 1 tour")
...
selenomancie = skill("Sélènomancie","ssp",TYPE_ARMOR,500,range=AREA_MONO,area=AREA_CIRCLE_2,effect=selenShield,cooldown=5,condition=[EXCLUSIVE,ASPIRATION,PROTECTEUR],use=INTELLIGENCE,emoji='<:selenomancie:949888629721927730>')
...
holiomancie = skill("Héliomancie",'sso',TYPE_INDIRECT_HEAL,500,effect=helioRegen,use=CHARISMA,area=AREA_CIRCLE_2,range=AREA_MONO,cooldown=5,condition=[EXCLUSIVE,ASPIRATION,VIGILANT],emoji='<:heliomancie:949888728116133889>')
...
exhibitionnisme = skill("Exhibitionnisme","ssm",TYPE_MALUS,69,range=AREA_MONO,area=AREA_CIRCLE_5,cooldown=7,effect=unHolly.effect[0],group=SKILL_GROUP_DEMON,use=CHARISMA,hpCost=20,emoji='<:exhibitionnisme:949893142402990120>',effPowerPurcent=80)
...
burningGround = skill("Terre Brûlée","ssh",TYPE_BOOST,750,emoji='<:burningGround:950256060923535370>',range=AREA_MONO,area=AREA_CIRCLE_2,effect=burningGroundEff,cooldown=7,hpCost=40,group=SKILL_GROUP_DEMON,description="Augmente les dégâts infligés par les alliés à portée au prix de 40% des PV actuels")
...
fascination = skill("Fascination","srs",TYPE_PASSIVE,price=750,emoji='<:evFas:951824800488230932>',effectOnSelf=eventFas,description="Vous octroi l'Eventail Fasninatoir. Lorsque vous ou vos alliés auront réalisés **10 coups critiques**, vous permet d'utiliser la compétence {0} __{1}__, qui inflige des dégâts aux ennemis autour de vous au début de votre tour".format(danceFas.emoji[0][0],danceFas.name),use=HARMONIE,quickDesc="Lorsque votre équipe a réalisé suffisament de coup critique, vous permet d'effectuer une puissante attaque autour de vous")
...
corGraFinal = skill("Chorégraphie Grâcieuse","corGraceFinal",TYPE_BOOST,500,range=AREA_MONO,area=aliceDanceFinal.area,effect=corGraEff,cooldown=aliceDance.cooldown,emoji='<:corGra:956700501095899217>',description="Augmente les statistiques de support de vos alliés proches durant 3 tours",effPowerPurcent=125)
...
propaUltLaunch = skill("Propagation Avancée","sqv",TYPE_INDIRECT_DAMAGE,750,range=AREA_MONO,emoji='<:propa2:993801072630046811>',area=AREA_ALL_ENEMIES,ultimate=True,cooldown=7,effect="me",effPowerPurcent=85,description="Après un tour de chargement, inflige <:est:884223390804766740> __Poison d'Estialba__ à tous les ennemis avec **35%** de sa puissance")
...
krystalisation = skill("Cristalisation","sqp",TYPE_ARMOR,500,range=AREA_MONO,effect=krystalisationEff,cooldown=7,emoji='<:cristalisation:989540878953619506>',description="Vous octrois une armure qui réduit également les dégâts indirects subis",condition=[EXCLUSIVE,ELEMENT,ELEMENT_EARTH])
...
fragmentation = skill("Fragmentation","sqn",TYPE_DAMAGE,500,50,AREA_CIRCLE_4,cooldown=5,damageOnArmor=3,erosion=65,condition=[EXCLUSIVE,ASPIRATION,TETE_BRULE],description="Inflige des dégâts à l'ennemi ciblé avec un grand pourcentage de réduction des PvMax adverses ainsi qu'un multiplicateur de dégâts sur l'armure conséquant")
...
machDeFer = skill("Mâchoire de Fer","sqm",TYPE_DAMAGE,500,condition=[EXCLUSIVE,ASPIRATION,ATTENTIF],power=80,useActionStats=ACT_INDIRECT,effect=[morsTempette,morsCaudique],cooldown=7,emoji='<:machFer:971787963795124235>')
...
lanceTox = skill("Lance-Toxine","sql",TYPE_INDIRECT_DAMAGE,500,area=AREA_CONE_3,effect=lanceToxEff,emoji='<:lanceTox:971789592145580103>',cooldown=5,condition=[EXCLUSIVE,'aspiration',ATTENTIF])
...
gigaFoudre = skill("Giga Foudre","sqk",TYPE_INDIRECT_DAMAGE,500,area=AREA_CIRCLE_1,effect=gigaFoudreEff,emoji='<:gigaFoudre:971789611087036487>',cooldown=5,condition=[EXCLUSIVE,'aspiration',SORCELER])
...
aurore2 = skill("Aurore II","sqh",TYPE_INDIRECT_HEAL,500,effect=aurore2Eff,cooldown=5,emoji='<:aurore:971787846639841310>',condition=[EXCLUSIVE,ASPIRATION,VIGILANT])
...
morsTempSkill = skill("Morsure de la Tempette","sqe",TYPE_DAMAGE,350,power=35,useActionStats=ACT_INDIRECT,effect=morsTempette,cooldown=5,condition=[EXCLUSIVE,ASPIRATION,ATTENTIF],emoji=morsTempette.emoji[0][0])
morsCaudiqueSkill = skill("Morsure Caudique","sqd",TYPE_DAMAGE,350,power=35,useActionStats=ACT_INDIRECT,effect=morsCaudique,cooldown=5,condition=[EXCLUSIVE,ASPIRATION,ATTENTIF],emoji=morsCaudique.emoji[0][0])
...
lightEarth = skill("Coeur de Lumière","sqa",TYPE_BOOST,500,cooldown=7,range=AREA_MONO,area=AREA_CIRCLE_3,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT],effect=lightEarthEff,emoji='<:lumHeart:980451074429288478>')
darkAmbEff = copy.deepcopy(lightEarthEff)
darkAmbEff.stat = ENDURANCE
darkAmb = skill("Missionaire des Ténèbres","spz",TYPE_BOOST,500,area=AREA_CIRCLE_4,range=AREA_MONO,cooldown=7,effect=darkAmbEff,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS],emoji='<:missionaire:980451008901701633>')
solarEruption = skill("Eruption Solaire","spy",TYPE_DAMAGE,500,100,area=AREA_ARC_1,cooldown=5,condition=[EXCLUSIVE,SECELEM,ELEMENT_SPACE],use=MAGIE)
...
roseeHeal = skill("Rosée matinale","sps",TYPE_BOOST,500,range=AREA_MONO,area=AREA_CIRCLE_3,cooldown=3,effect=roseeHealEff,effectAroundCaster=[TYPE_HEAL,AREA_CIRCLE_2,35],description="Augmente l'esquive des alliés à portée tout en les soignants un peu")
...
mve2 = skill("Mors Vita Est Eternatum","spg",TYPE_PASSIVE,0,effectOnSelf=mve2Eff,description="Lorsque vous êtes vaincu, vous vous réanimés directement avec un certain pourcentage de vos PV max",emoji=kikuRes.emoji,quickDesc="Vous permet de vous réanimer vous même une fois vaincu")
...
recall = skill("Rappel","soq",TYPE_RESURECTION,750,power=150,use=CHARISMA,emoji="<:rewind:984519329993347082>",cooldown=7,area=AREA_CIRCLE_3,effect=jetLagEff,condition=[EXCLUSIVE,ELEMENT,ELEMENT_TIME],description="Réanime les alliés vaincus, mais leur inflige <:jetLagB:984519444082610227> __Déphasage Intemporel__")
...
divineCircle = skill("Cercle Sacré",getAutoId(elemRuneSkill[-1].id,True),TYPE_BOOST,500,effect=divineCircleEff,area=AREA_CIRCLE_3,cooldown=5,maxHpCost=10,emoji='<:divineCercle:989532178109042698>')
...
raisingPheonixLaunch = skill("Envolée du Phénix",getAutoId(plumRem.id,True),TYPE_DAMAGE,750,150,area=AREA_CIRCLE_5,emoji='<:rizePhenix:992940415080726688>',range=AREA_MONO,ultimate=True,cooldown=7,effectAroundCaster=[TYPE_RESURECTION,AREA_CIRCLE_4,150],use=MAGIE,useActionStats=ACT_DIRECT,description="Après un tour de chargement, Inflige des dégâts aux ennemis alentours tout en réanimant les alliés à portée")
...
dmonProtect = skill("Protection Démoniaque",getAutoId(fairyBomb.id,True),TYPE_BOOST,500,effect=dmonProtectEff,cooldown=5,hpCost=15,range=AREA_DONUT_5,description="Réduit les dégâts subis par un allié et sa probabilité de bloquer une attaque pendant **3** tours",emoji='<:proDmon:994026279680606258>')
...
hemoBomb = skill("Cogitation Hémorragique",getAutoId(dmonProtect.id,True),TYPE_INDIRECT_DAMAGE,750,effect=hemoBombEff,cooldown=7,emoji='<:bloodyBomb:992499935738089592>',description="Après deux tours ou si l'ennemi ciblé est vaincu, déclanche une détonatin à sa position lui infligeant des dégâts ainsi qu'à vos ennemis allignés.\nLes entités affectés subissent l'effet __Hémorragie__ (25%) pendant 3 tours")
barrage = skill("Barrage",getAutoId(hemoBomb.id,True),TYPE_DAMAGE,500,40,range=AREA_CIRCLE_4,area=AREA_CONE_2,cooldown=5,repetition=3,emoji="<:barrage:994031489773748265>",sussess=40)
...
tripleAttaque2 = skill("Triple Attaque",getAutoId(liberation.id,True),TYPE_DAMAGE,500,50,range=AREA_CIRCLE_3,cooldown=5,description="Inflige trois attaques sur l'ennemi ciblé. Le dernier coup repousse la cible",knockback=2,condition=[EXCLUSIVE,ASPIRATION,POIDS_PLUME])
...
tripleAttaque1.power, tripleAttaque1.effectOnSelf, tripleAttaque1.knockback, tripleAttaque1.replay = 45, tripleAttaque2cast, 0, True
...
tripleAttaque.power, tripleAttaque.effectOnSelf, tripleAttaque.replay = 40, tripleAttaque1cast, True

arsenic = effect("Arsenic","arsenic",STRENGTH,power=25,turnInit=3,lvl=3,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_START_OF_TURN,stackable=True,emoji='<:arsenic:995361025773817916>')
acidRain1 = skill("Pluie Acide",getAutoId(tripleAttaque.id,True),TYPE_INDIRECT_DAMAGE,750,range=AREA_MONO,area=AREA_ALL_ENEMIES,emoji='<:acidRain:995360946241417388>',minTargetRequired=3,effect=arsenic,effPowerPurcent=propaUltLaunch.effPowerPurcent,ultimate=True,cooldown=propaUltLaunch.cooldown,description="Après un tour de chargement, inflige __Arsenic__ à tous les ennemis")
...
acidRain.effect, acidRain.effectOnSelf = [None], acidRainCast
coroRocket = skill("Fusée Corosive",getAutoId(acidRain.id,True),TYPE_INDIRECT_DAMAGE,500,effect=arsenic,area=AREA_CONE_2,cooldown=poisonus.cooldown,emoji='<:coroRocket:995360986603196417>')
coroMissile1 = skill("Missile corrosif",getAutoId(coroRocket.id,True),TYPE_INDIRECT_DAMAGE,750,effect=arsenic,area=AREA_CONE_3,minTargetRequired=2,cooldown=7,emoji='<:coroMissile:995361003581739018>')
...
coroMissile.effect, coroMissile.effectOnSelf = [None], coroMissileCast
coroShot = skill("Tir corrosif",getAutoId(coroMissile.id,True),TYPE_DAMAGE,500,80,area=AREA_LINE_3,effect=arsenic,cooldown=5,emoji='<:coroShot:995360968697716837>')

quadFleau = skill("Quadruple Fléau",getAutoId(coroShot.id,True),TYPE_INDIRECT_DAMAGE,750,effect=["me",coroWind,"mx",arsenic],effPowerPurcent=50,cooldown=7,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS])
...
windDanceEff = effect("Soyokaze","windDanceEff",turnInit=-1,unclearable=True,emoji='<:liaCounter:998001563379437568>',reject=["liaKatanaEff"],counterProb=25,description="Vous octroi **25%** de chance d'effectuer une **contre-attaque** en esquivant une attaque d'un adversaire se trouvant à 3 cases ou moins de vous")
...
taureauFury = skill("Furie du Taureau","geghthz",TYPE_DAMAGE,500,power=37,setAoEDamage=True,repetition=3,sussess=100,area=AREA_CIRCLE_2,range=AREA_MONO,cooldown=7,description="Inflige trois attaques aux ennemis à portée de mêlée avec une précision accrue")
gemeauSynEff = copy.deepcopy(dmgUp)
gemeauSynEff.power = 15
gemeauSyn = skill("Synergie des Jumeaux","hehge",TYPE_BOOST,500,effect=gemeauSynEff,effectOnSelf=gemeauSynEff,cooldown=7,replay=True,description="Augmente vos propres dégâts infligés ainsi que ceux de l'allié ciblé de **15%** pendant un tour et vous permet de rejouer")
...
lionRoar = skill("Rugissement du Lion","hehehe",TYPE_MALUS,500,range=AREA_MONO,area=AREA_DONUT_4,effect=lionRoarEff,cooldown=7,description="Réduit les statistiques des ennemis alentours pendant un tour")
...
vierge = skill("Bénidiction de la Vièrge","rgege",TYPE_BOOST,500,effect=viergeSkillEff,cooldown=7,description="Accroi la Résistance et réduit les dégâts indirects subis de l'allié ciblé")
...
balanceBoost = skill("Equilibre de la Balance","rejgikjhg",TYPE_BOOST,500,area=AREA_CIRCLE_5,range=AREA_MONO,effect=balanceBoostEff,cooldown=7,description="Augmente légèrement les statistiques principaux de vos alliés proches")
...
scorpionByte = skill("Piqûre du Scorpion","gsg",TYPE_DAMAGE,500,power=65,use=MAGIE,effect=scorpVenum,cooldown=7,description="Inflige des dégâts à l'ennemi ciblé et l'empoisonne pendant 2 tours",useActionStats=ACT_INDIRECT)
...
verseau = skill("Omniprésence du Verseau","hehhee",TYPE_HEAL,500,50,area=AREA_CIRCLE_2,effect=verseauShield,range=AREA_CIRCLE_4,cooldown=7,description="Soigne les alliés ciblés et leur octroi une armure",use=INTELLIGENCE)
fishHot = effect("Grâce du Poisson","fishGrace",CHARISMA,power=22,turnInit=3,type=TYPE_HEAL,trigger=TRIGGER_START_OF_TURN)
fishGrace = skill("Grâce du Poisson","heheheh",TYPE_INDIRECT_HEAL,500,effect=fishHot,area=AREA_CIRCLE_3,range=AREA_MONO,cooldown=7,description="Applique un effet de régénération sur vous et vos alliés proches")
lightEstoc = skill("Estoc Lumineuse",getAutoId("smh",True),TYPE_DAMAGE,500,power=80,range=AREA_CIRCLE_1,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT],effectAroundCaster=[TYPE_HEAL,AREA_CIRCLE_4,40],cooldown=5,description="Inflige des dégâts aux ennemis ciblés et vous soigne ainsi que vos alliés alentours",use=CHARISMA,useActionStats=ACT_HEAL,area=AREA_LINE_2)
...
lightTranche = skill("Tranche Lumineuse",getAutoId(lightEstoc.id,True),TYPE_DAMAGE,500,power=85,range=AREA_CIRCLE_1,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT],effectAroundCaster=[TYPE_ARMOR,AREA_CIRCLE_4,lightTrancheShield],cooldown=5,description="Inflige des dégâts aux ennemis ciblés et vous procure ainsi qu'à vos alliés alentours une armure",use=INTELLIGENCE,useActionStats=ACT_SHIELD,area=AREA_ARC_1)
lightShot = skill("Tir Lumineux",getAutoId(lightTranche.id,True),TYPE_DAMAGE,500,power=70,range=AREA_CIRCLE_5,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT],effectAroundCaster=[TYPE_HEAL,AREA_CIRCLE_4,35],cooldown=5,description="Inflige des dégâts aux ennemis ciblés et vous soigne ainsi que vos alliés alentours",use=CHARISMA,useActionStats=ACT_HEAL,area=AREA_LINE_2)
...
sillage = skill("Sillage",getAutoId(cycloneEcarlate.id,True),TYPE_DAMAGE,500,sillageEff.power+10,use=MAGIE,area=AREA_CIRCLE_1,range=AREA_CIRCLE_4,effect=sillageEff,cooldown=7,emoji=sillageEff.emoji[0][0],description="Inflige des dégâts à l'ennemi ciblé et lui inflige un effet de poison qui infligera des dégâts en zone durant les deux prochains tours",condition=[EXCLUSIVE,ELEMENT,ELEMENT_AIR])
...
FOCUSJAUGEGAINPERTURN, FOCUSJAUGEGAINPERSINGLETARGET, FOCUSJAUGEGAINPERMULTITARGET = 5, 10, 4
focusJauge = effect("Jauge Focus","focusJauge",emoji='<:reasable:983595291930415124>',jaugeValue=lysJauge.jaugeValue,turnInit=-1,unclearable=True,description="La Jauge Focus augmente dans 3 cas particuliers :\n> +**{0} pts** en débutant son tour en étant en vie\n> +**{1} pts** en touchant un ennemi avec une arme ou compétence monocible infligeant des dégâts physiques\n> +**{2} pts** en touchant un ennemi avec une arme ou compétence infligeant des dégâts en zone physiques ou une arme infligeant plusieurs attaques physiques en un tour\n\nUne compétence utilisant la Jauge Focus ne peut pas en générer".format(FOCUSJAUGEGAINPERTURN,FOCUSJAUGEGAINPERSINGLETARGET,FOCUSJAUGEGAINPERMULTITARGET))
...
anchor = skill("Ancre Aérienne",getAutoId(drill.id,True),TYPE_DAMAGE,750,power=125,jaugeEff=focusJauge,minJaugeValue=50,maxJaugeValue=50,effect=anchorEff,description="Inflige des dégâts à l'ennemi ciblé et restrain ses déplacements pendant 3 tours",emoji='<:anchor:1003645801421099018>',cooldown=3)
...
lowBlow = skill("Coup Bas",getAutoId(tablAdvBaseElemSkills[-1].id,True),TYPE_DAMAGE,750,power=45,range=AREA_CIRCLE_2,use=ENDURANCE,effect=lightStun,cooldown=5,maxPower=75,description="Inflige des dégâts et étourdi l'ennemi ciblé.\nLa puissance de la compétence est augmentée contre un boss Stand Alone, un boss de Raid ou certains ennemis",emoji='<:lowblow:1005127208412651591>')
legStrike = skill("Fauchage de Jambes",getAutoId(lowBlow.id,True),TYPE_DAMAGE,750,80,AREA_CIRCLE_2,effect=lightStun,cooldown=7,description="Inflige des dégâts à étourdi l'ennemi ciblé",emoji='<:legsStrike:1005127243401539645>')
...
provoke = skill("Provocation",getAutoId(legStrike.id,True),TYPE_MALUS,500,effect=provokeEff,cooldown=5,pull=4,range=AREA_CIRCLE_4,description="Augmente considérablement la probabilité d'être ciblé par l'adversaire désigné et attire celui-ci vers vous",emoji=provokeEff.emoji[0][0])

fanRevolEff = effect("Poussé Céleste","fanRevolEff",CHARISMA,strength=10,agility=5,turnInit=2)
...
fanReverse = skill("Eventail Virvoltant Inversé",fanRevol.id,TYPE_DAMAGE,power=85,area=AREA_ARC_3,emoji='<:revFlyingFan:1003309639280111647>',needEffect=[fanReverseReady],effectOnSelf=fanAscendReady)
fanVir = skill("Eventail Virvoltant",fanRevol.id,TYPE_DAMAGE,power=70,area=AREA_ARC_2,emoji='<:flyingFan:1003309655444963408>',effectOnSelf=fanReverseReady)
...
demonLand = skill("Terre Démoniaque",getAutoId(comboFanCelest.id,True),TYPE_BOOST,500,cooldown=7,effect=[demonLandEff1,demonLandEff2],area=AREA_DONUT_3,range=AREA_MONO,group=SKILL_GROUP_DEMON,hpCost=20,emoji='<:dmonland:1006455391342829599>')
...
demonArmor = skill("Armure Démoniaque",getAutoId(demonLand.id,True),TYPE_ARMOR,750,effect=demonArmorEff,cooldown=5,group=SKILL_GROUP_DEMON,hpCost=10,emoji='<:dmonArmor:1006455374464946226>')
demonArmorEff2 = effect("Armure Démoniaque Avancée","demonArmor2",INTELLIGENCE,overhealth=50,turnInit=3,type=TYPE_ARMOR,trigger=TRIGGER_DAMAGE,emoji='<:dmonArmor:1006455374464946226>')
demonArmor2 = skill("Armure Démoniaque Avancée",getAutoId(demonArmor.id,True),TYPE_ARMOR,750,effect=demonArmorEff,cooldown=5,group=SKILL_GROUP_DEMON,hpCost=15,area=AREA_CIRCLE_3,range=AREA_MONO,emoji='<:dmonArmor:1006455374464946226>')
demonConstEff = copy.deepcopy(absEff)
demonConstEff.power, demonConstEff.turnInit = 20, 3
demonConst = skill("Constitution Démoniaque",getAutoId(demonArmor2.id,True),TYPE_BOOST,500,area=AREA_CIRCLE_3,range=AREA_MONO,effect=demonConstEff,cooldown=5,group=SKILL_GROUP_DEMON,hpCost=10,emoji='<:dmonConst:1006455408250081310>')

SOULJAUGEGAINENEMYDEATH, SOULJAUGEGAINALLYDEATH, SOULJAUGEGAINSTARTOFTURN = 10, 5, 2
soulJauge = effect("Jauge d'Âme","soulJauge",emoji='<:soulJauge:983595647292829716>',jaugeValue=[["<:lej:980923743960461412>","<:mej:980923762016927764>","<:rej:980923786650075228>"],["<:sjl:983600118152441886>","<:sjm:983600154252808202>","<:sjr:983600136125022248>"]],turnInit=-1,unclearable=True,description="La Jauge d'Âme se remplie dans 3 conditions :\n> - **+ {0} pts** en débutant son tour en étant en vie\n> - **+ {1} pts** lorsqu'un ennemi est vaincu\n> - **+ {2} pts** lorsqu'un allié est vaincu\n\nCertaines compétences et effets peuvent également replir la jauge d'âme".format(SOULJAUGEGAINSTARTOFTURN, SOULJAUGEGAINENEMYDEATH, SOULJAUGEGAINALLYDEATH))
noneLem = effect("Repos du Néan","noneLem",silent=True,turnInit=7,type=TYPE_MALUS,emoji='<:noneLem:1007588165512593431>')
...
soulFauchCroix = skill("Fauchage Croisé",comunio.id,TYPE_DAMAGE,power=110,range=AREA_CIRCLE_3,area=AREA_ARC_2,sussess=99,emoji="<:faucheCroix:937807152058343474>",needEffect=soulFauchCroixReady)
soulFauchNeanReady = effect("Fauchage du Néan Préparé","soulFauchNean",turnInit=3,emoji='<:faucheNean:937807137306976337>')
soulFauchNean = skill("Fauchage du Néan",comunio.id,TYPE_DAMAGE,power=100,range=AREA_CIRCLE_3,area=AREA_ARC_1,sussess=120,emoji="<:faucheNean:937807137306976337>",needEffect=soulFauchNeanReady,effectOnSelf=soulFauchCroixReady)
...
linceuilLemure = skill("Linceuil de Lémure",comunio.id,TYPE_BOOST,range=AREA_MONO,url='https://media.discordapp.net/attachments/1003025632415973510/1007612026585092148/20220812_132659.gif',cooldown=0,area=AREA_MONO,emoji='<:linc:1007583748491051058>',effect=[lincEff,comunioReady,soulFauchNeanReady,tranchNeanReady,noneLem],rejectEffect=noneLem,jaugeEff=soulJauge,minJaugeValue=35,maxJaugeValue=35,description="Consomme une partie de votre jauge d'âme pour augmenter vos dégâts infligés de 10% pendant 3 tours et vous permet d'effectuer plusieurs nouvelles compétences sur la même durée")
...
convictionVigilantEff = effect("Conviction du Vigilant","convVigil",CHARISMA,power=20,type=TYPE_INDIRECT_HEAL,unclearable=True,turnInit=-1,description="Lorsque le porteur utilise son arme ou une compétence de dégâts directs, le soigne légèrement",emoji="<:regenVigil:982210651353137153>")
...
convictionProtecArmor = effect("Armure","convicProAmor",INTELLIGENCE,overhealth=20,stackable=True,lightShield=True,type=TYPE_ARMOR,trigger=TRIGGER_DAMAGE)
convicProEff = effect("Conviction du Protecteur","convicPro",INTELLIGENCE,callOnTrigger=convictionProtecArmor,turnInit=-1,unclearable=True,emoji='<:proOHArmor:909278718575394837>',description="Octroi une armure légère au porteur lorsque celui-ci utilise son arme ou une compétence de dégâts direct")
convicPro = skill("Conviction du Protecteur",getAutoId(convictionVigilante.id,True),TYPE_PASSIVE,500,effectOnSelf=convicProEff,emoji=convicProEff.emoji[0][0],condition=[EXCLUSIVE,ASPIRATION,PROTECTEUR],quickDesc="Vous octroi une armure quand vous utiliser votre arme ou une compétence offensive")
convictEnchtEff = effect("Conviction de l'Enchanteur","convVigil",MAGIE,power=30,type=TYPE_INDIRECT_HEAL,unclearable=True,turnInit=-1,description="Lorsque le porteur utilise son arme ou une compétence de dégâts directs, le soigne légèrement",emoji="<:AltyRegen:994748551932428319>")
convictEncht = skill("Conviction de l'Enchanteur",getAutoId(convicPro.id,True),TYPE_PASSIVE,500,effectOnSelf=convictEnchtEff,emoji='<:AltyRegen:994748551932428319>',quickDesc="Vous soigne légèrement lorsque vous utilisez une option offensive",condition=[EXCLUSIVE,ASPIRATION,ENCHANTEUR])
convictTeteArmor = effect("Armure","convicTetAmor",STRENGTH,overhealth=25,stackable=True,lightShield=True,type=TYPE_ARMOR,trigger=TRIGGER_DAMAGE)
convictTetEff = effect("Conviction du Tête Brulée","convicPro",STRENGTH,callOnTrigger=convictTeteArmor,turnInit=-1,unclearable=True,emoji='<:idoOHArmor:909278702783836170>',description="Octroi une armure légère au porteur lorsque celui-ci utilise son arme ou une compétence de dégâts direct")
...
megaDark1 = skill("Méga Cécité",getAutoId(convictTet.id,True),TYPE_INDIRECT_DAMAGE,dark1.price,effect=megaDark1Eff,emoji=dark1.emoji,cooldown=dark1.cooldown,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS])
megaDark2 = skill("Méga Obscurité",getAutoId(megaDark1.id,True),TYPE_INDIRECT_DAMAGE,dark2.price,effect=megaDark2Eff,emoji=dark2.emoji,cooldown=dark2.cooldown,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS])
megaDark3 = skill("Méga Pénombre",getAutoId(megaDark2.id,True),TYPE_INDIRECT_DAMAGE,dark3.price,effect=megaDark3Eff,emoji=dark3.emoji,cooldown=dark3.cooldown,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS])
gigaDark1 = skill("Giga Cécité",getAutoId(megaDark3.id,True),TYPE_INDIRECT_DAMAGE,dark1.price,effect=gigaDark1Eff,emoji=dark1.emoji,cooldown=dark1.cooldown,area=AREA_CIRCLE_1,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS])
gigaDark2 = skill("Giga Obscurité",getAutoId(gigaDark1.id,True),TYPE_INDIRECT_DAMAGE,dark2.price,effect=gigaDark2Eff,emoji=dark2.emoji,cooldown=dark2.cooldown,area=AREA_CIRCLE_1,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS])
gigaDark3 = skill("Giga Pénombre",getAutoId(gigaDark2.id,True),TYPE_INDIRECT_DAMAGE,dark3.price,effect=gigaDark3Eff,emoji=dark3.emoji,cooldown=dark3.cooldown,area=AREA_CIRCLE_1,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS])
...
gigaLight1 = skill("Giga {0}".format(light1.name),getAutoId(megaLight3.id,True),TYPE_ARMOR,light1.price,effect=gigaLight1Eff,cooldown=light1.cooldown,emoji=light1.emoji,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT])
gigaLight2 = skill("Giga {0}".format(light2.name),getAutoId(gigaLight1.id,True),TYPE_ARMOR,light2.price,effect=gigaLight2Eff,cooldown=light2.cooldown,emoji=light2.emoji,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT])
gigaLight3 = skill("Giga {0}".format(light3.name),getAutoId(gigaLight2.id,True),TYPE_ARMOR,light3.price,effect=gigaLight3Eff,cooldown=light3.cooldown,emoji=light3.emoji,condition=[EXCLUSIVE,ELEMENT,ELEMENT_LIGHT])
...
seraphStrike = skill("Frappe Séraphique", getAutoId(toxicon.id,True),TYPE_DAMAGE,500,power=50,range=AREA_CIRCLE_3,tpCac=True,effectAroundCaster=[TYPE_BOOST,AREA_CIRCLE_3,seraphStrikeEff],cooldown=5,use=INTELLIGENCE,useActionStats=ACT_BOOST,condition=[EXCLUSIVE,ASPIRATION,MASCOTTE],description="Vous permet de sauter sur un ennemi en lui infligeant des dégâts, puis réduit les dégâts reçus par vous et vos alliés alentours pendant 3 tours")

# Skill
skills: List[skill] = [convictionVigilante,convicPro,convictTet,convictEncht,megaDark1,megaDark2,megaDark3,gigaDark1,gigaDark2,gigaDark3,megaLight1,megaLight2,megaLight3,gigaLight1,gigaLight2,gigaLight3,miracle,rivCel,toxicon,seraphStrike,
...
] + elemArrowSkill + elemRuneSkill + horoSkills + tablAdvBaseElemSkills
