...
squidRollArmor = effect("Armure","squidRollArmor", stat=PURCENTAGE,overhealth=5,type=TYPE_ARMOR,trigger=TRIGGER_DAMAGE,lightShield=True,emoji=armor.emoji)
...
energyDrain = skill("Aspiration d'énergie",getAutoId(squidRoll.id,True),TYPE_DAMAGE,power=50,replay=True,use=MAGIE,emoji='<:energyDrain:1130205922929684490>',lifeSteal=35,description="Inflige des dégâts à l'ennemi ciblé en absorbant une partie des dégats infligés, remplie votre {0} {1} de {2} points et vous permet de rejouer votre tour".format(etherJauge.emoji[0][0],etherJauge.name,etherJauge.jaugeValue.conds[1].value),cooldown=3)
energySiphon = skill("Syphon d'énergie",energyDrain.id,TYPE_DAMAGE,power=40,replay=True,area=AREA_CIRCLE_1,use=MAGIE,emoji='<:syphon:1130205955846590505>',lifeSteal=20,description="Inflige des dégâts aux ennemis ciblés en absorbant une partie des dégats infligés, remplie votre {0} {1} de {2} points et vous permet de rejouer votre tour".format(etherJauge.emoji[0][0],etherJauge.name,etherJauge.jaugeValue.conds[1].value),cooldown=3)
...
denoumentEff1 = effect("Vulnérabilité Indirecte","traqnardEff",inkResistance=-50,turnInit=1,type=TYPE_MALUS,description="Augmente les dégâts indirects reçus par le porteur",stackable=True)
denoumentEff2 = effect("Vulnérabilité Augmentée","denoumentEff2",block=-25,dodge=-20,aggro=50,emoji='<:denoument:1163831222087200822>')
denoument = skill("Dénouement",getAutoId(royalShield.id,True),TYPE_MALUS,750,effects=[denoumentEff1,denoumentEff2],replay=True,cooldown=5,range=AREA_CIRCLE_1,emoji=denoumentEff2.emoji[0][0],description="Augmente drastiquement les dégâts indirectes et l'aggression de l'ennemi ciblé tout en réduisant sa probabilité de contrer ou esquiver une attaque durant un tour et vous permet de rejouer votre tour")
...
propagEffTrig = effect("Propagateur","propagTrig", stat=HARMONIE,power=40,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_INSTANT,emoji='<:propagMaster:1157361864209088684>',lifeSteal=50)
...
gwenyQCombo1 = skill("Arrachage Renforcée",gwenyQCombo3.id,TYPE_DAMAGE,power=30,armorConvert=50,range=AREA_CIRCLE_1,armorSteal=50,aoeArmorConvert=35,replay=True,use=INTELLIGENCE,emoji=gwenyQCombo1_r.emoji[0][0],cooldown=1,description="Inflige des dégâts à l'ennemi ciblé en vous octroyant de l'armure en fonction des dégâts infligés et vous permet de rejouer votre tour",needEffect=gwenyQCombo1_r,effectOnSelf=gwenyQCombo2_r,rejectEffect=[gwenyQCombo2_r,gwenyQCombo3_r])
...
altyQCombo1 = skill("Arrachage Revifiante",altyQCombo3.id,TYPE_DAMAGE,power=30,lifeSteal=50,range=AREA_CIRCLE_1,aoeLifeSteal=35,damageOnArmor=1.2,replay=True,use=CHARISMA,emoji=altyQCombo1_r.emoji[0][0],cooldown=1,description=altyQCombo3.description,needEffect=altyQCombo1_r,effectOnSelf=altyQCombo2_r,rejectEffect=[altyQCombo2_r,altyQCombo3_r])
...
propagDelEffTrig = effect("Propagateur Délayé","propagTrig", stat=HARMONIE,power=int(propagEffTrig.power*0.65),type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_END_OF_TURN,emoji='<:transgression:1154369057630453780>',lifeSteal=50,turnInit=2,stackable=True)
propagDelEff = effect("Propagateur Délayé","propag",callOnTrigger=propagDelEffTrig,turnInit=-1,unclearable=True,emoji='<:stopUndoingMyDoings:1154368777476132864>')
...
trainePlumeEff.power, trainePlumeEff.stat = 30, STRENGTH
