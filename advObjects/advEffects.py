...
coffee = effect("Caféiné","lb",CHARISMA,strength=10,endurance=10,reject=["mc"],description="Boost la force et l'endurance de tous les alliés",emoji=uniqueEmoji("<:coffee:867538582846963753>"))
the = effect("Théiné","lc",CHARISMA,intelligence=10,magie=10,reject=["mc"],description="Boost l'agilité et la précision de tous les alliés",emoji=uniqueEmoji('<:the:867538602644602931>'))
...
stopAttacking = effect("Stop attacking or draw 25","lq",INTELLIGENCE,trigger=TRIGGER_DEALS_DAMAGE,type=TYPE_INDIRECT_DAMAGE,power=25,emoji=emojiMalus,description="Vous jouez votre jocker !\nSi le porteur de l'état attaque, il subit 25 dégâts fixe")
...
menthe = effect("Mentiné","lu",CHARISMA,percing=5,resistance=5,critical=10,reject=["mc"],description="Boost la résistance, la pénétration et le critique de vos alliés",emoji=uniqueEmoji('<:menthe:867538622797054042>'))
...
stuned = effect("Etourdi","mb",emoji=uniqueEmoji('<:stun:882597448898474024>'),type=TYPE_MALUS,turnInit=3,stun=True,description="L'utilisation d'un gros sort magique vous a vidé de votre énergie")
defensive = effect("Orbe défensif","md",emoji=sameSpeciesEmoji('<:orbe1:873725384359837776>','<:orbe2:873725400730202123>'),resistance=5,overhealth=55,description='Donne de l\'armure à un allié',stat=INTELLIGENCE,trigger=TRIGGER_DAMAGE,turnInit=1)
...
estal = effect("Poison d'Estialba","me",emoji=uniqueEmoji('<:est:884223390804766740>'),turnInit=3,stat=MAGIE,description="Un virulant poison, faisant la sinistre renommée des fées de cette île",trigger=TRIGGER_START_OF_TURN,stackable=True,power=35,type=TYPE_INDIRECT_DAMAGE,lvl=3)
missiles = effect("Ciblé","mf",emoji=uniqueEmoji('<:tentamissile:884757344397951026>'),stat=STRENGTH,description="Au début de son tour, le ciel tombera sur la tête du Ciblé et ses alliés proches !",trigger=TRIGGER_START_OF_TURN,power=35,type=TYPE_INDIRECT_DAMAGE,area=AREA_CIRCLE_1,stackable=True)
...
isoled = effect("Isolé","ml",emoji=uniqueEmoji('<:selfProtect:887743151027126302>'),description="S'isoler mentalement pôur ne pas faire attention au dégâts",overhealth=120,stat=INTELLIGENCE,trigger=TRIGGER_DAMAGE)
nouil = effect("Œuil de Linx","mm",PRECISION,emoji=uniqueEmoji('<:noeuil:887743235131322398>'),precision=10,critical=3,turnInit=2)
...
onstageeff = effect("Euphorie","mr",CHARISMA,strength=10,intelligence=10,charisma=10,magie=10,resistance=3,description="C'est le moment de tout donner !",emoji=uniqueEmoji('<:alice:893463608716062760>'))
...
lighteff = effect("Illuminé","mu",INTELLIGENCE,overhealth=80,type=TYPE_ARMOR,trigger=TRIGGER_DAMAGE,emoji=uniqueEmoji('<:illumi1:902008944887746611>'))
...
ferociteEff = effect("Férocité","ne",magie=10,aggro=15,resistance=10,turnInit=-1,emoji=uniqueEmoji('<:ferocite:899790356315512852>'),description="Vos grands airs augmentent les chances d'être attaqué par les ennemis",unclearable=True,reject=tankStans)
defiEff = effect("Défi","nf",strength=10,aggro=15,resistance=10,turnInit=-1,emoji=uniqueEmoji('<:defi:899793973873360977>'),description="Vos grands airs augmentent les chances d'être attaqué par les ennemis",unclearable=True,reject=tankStans)
royaleGarde = effect("Garde Royale","ng",intelligence=10,aggro=15,resistance=10,turnInit=-1,emoji=uniqueEmoji('<:gardeRoyale:899793954315321405>'),description="Vos grands airs augmentent les chances d'être attaqué par les ennemis",unclearable=True,reject=tankStans)
ironWill = effect("Volontée de Fer","nh",charisma=10,aggro=15,resistance=10,turnInit=-1,emoji=uniqueEmoji('<:ironwill:899793931762565251>'),description="Vos grands airs augmentent les chances d'être attaqué par les ennemis",unclearable=True,reject=tankStans)
...
bleeding2 = effect("Hémoragie II","nr",STRENGTH,power=int(bleeding.power/3),type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_START_OF_TURN,stackable=True,emoji=uniqueEmoji('<:bleeding2:900329311955984456>'))
...
blackHoleEff = effect("Singularité","ol",aggro=35,turnInit=-1,unclearable=True,reject=tankStans,description="Augmente considérablement les chances d'être pris pour cible par l'adversaire",emoji='<:blackHole:906195944406679612>')
...
idoOHEff = effect("Apothéose","idoOHEff",emoji=uniqueEmoji('<:IdoOH:909278546172719184>'),turnInit=-1,power=30,description="Vous motivez vos alliés plus que jamais !\n\n**{0}**% de vos sur-soins sont donnés en armure\n\n__<:idoOHArmor:909278702783836170> Overheath - Idole :__\nArmure Légère : Les armures légères n'absorbent pas de dégâts supplémentaires lors de leurs destructions\nEffet Remplaçable : Les effets remplaçables sont remplassés si le même effet avec une meilleure puissance est donné",unclearable=True,reject=healStans)
proOHEff = effect("Protection anticipée","proOHEff",emoji=uniqueEmoji('<:proOH:909278525528350720>'),turnInit=-1,power=30,description="Vous avez réusi à surpasser vos limites !\n\n**{0}**% de vos sur-soins sont donnés en armure\n\n__<:proOHArmor:909278718575394837> Overheath - Protecteur :__\nArmure Légère : Les armures légères n'absorbent pas de dégâts supplémentaires lors de leurs destructions\nEffet Remplaçable : Les effets remplaçables sont remplassés si le même effet avec une meilleure puissance est donné",unclearable=True,reject=healStans)
altOHEff = effect("Bénédiction","altOHEff",emoji=uniqueEmoji('<:altOH:909278509145395220>'),turnInit=-1,power=50,description="Votre dévotion pour vos alliés vous permet de passer à la vitesse supérieur !\n\n**{0}**% de vos sur-soins sont donnés en armure\n\n__<:aoa:909278749143490601>  Overheath - Altruiste :__\nArmure Légère : Les armures légères n'absorbent pas de dégâts supplémentaires lors de leurs destructions\nEffet Remplaçable : Les effets remplaçables sont remplassés si le même effet avec une meilleure puissance est donné",unclearable=True,reject=healStans)
lightAura2ActiveEff = effect("Charges lumineuses","lightaura2Ac",CHARISMA,trigger=TRIGGER_AFTER_DAMAGE,turnInit=2,stackable=True,type=TYPE_INDIRECT_HEAL,power=10,lvl=5,description="Les 5 prochaines attaques que vous receverez déclancheront un effet de soins autour de vous",area=AREA_CIRCLE_2,emoji=lightAuraEffect.emoji)
...
proMalus = effect("Protecteur - Malus","nb",HARMONIE,strength=-3,magie=-3,type=TYPE_MALUS,silent=True,stackable=True,emoji=uniqueEmoji('<:proMalus:903137298001047573>'),turnInit=2)
