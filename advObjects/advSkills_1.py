...
estial = effect("Poison d'Estialba","me",emoji=uniqueEmoji('<:est:884223390804766740>'),turnInit=3,stat=MAGIE,description="Un virulant poison, faisant la sinistre renommée des fées de cette île",trigger=TRIGGER_START_OF_TURN,stackable=True,power=30,type=TYPE_INDIRECT_DAMAGE,lvl=3)
estal2 = effect("Poison d'Estialba II","nq",MAGIE,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_START_OF_TURN,power=int(estial.power/3),turnInit=3,emoji='<:estialba2:900329155974008863>',stackable=True,lvl=3)
bleeding = effect("Hémorragie","mx",STRENGTH,power=40,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_START_OF_TURN,turnInit=3,stackable=True,lvl=3,emoji=uniqueEmoji('<:ble:887743186095730708>'))
...
explosion = skill("Explosion","KBOOM",TYPE_DAMAGE,1000,power=225,percing=0,ultimate=True,cooldown=12,area=AREA_CIRCLE_2,shareCooldown=True,effectOnSelf=explosionSilentEff,use=MAGIE,emoji='<a:explosion:882627170944573471>',damageOnArmor=0.8,url='https://media.discordapp.net/attachments/933783831272628356/934066959040016404/20220121_134758.gif',description="Inflige des dégâts colosaux dans une grande zone")
...
poisonus = skill("Vent empoisonné","zd",TYPE_INDIRECT_DAMAGE,500,emoji='<:estabistia:883123793730609172>',effects=estial.id,cooldown=5,area=AREA_CIRCLE_1,use=MAGIE,message="{0} propage un {1} autour de {2} :")
...
secondWind = skill("Second Souffle","yo",TYPE_HEAL,500,300,AREA_MONO,emoji='<:secondWind:897634132639756310>',cooldown=7,use=ENDURANCE)
...
onstageeff = effect("Euphorie","mr",CHARISMA,strength=15,intelligence=15,charisma=15,magie=15,resistance=5,emoji=uniqueEmoji('<:alice:893463608716062760>'))
onstage = skill("En scène ","yf",TYPE_BOOST,750,0,AREA_MONO,[EXCLUSIVE,ASPIRATION,IDOLE],True,effects=onstageeff,emoji='<:alice:893463608716062760>',area=AREA_CIRCLE_3,use=CHARISMA,cooldown=7)
icelance = skill("Lame glacée","ye",TYPE_DAMAGE,500,180,AREA_DIST_6,[EXCLUSIVE,ELEMENT,ELEMENT_WATER],True,emoji='<:emoji_47:893471252537298954>',cooldown=5,use=MAGIE)
rocklance = skill("Lame rocheuse","yd",TYPE_DAMAGE,500,180,AREA_CIRCLE_3,[EXCLUSIVE,ELEMENT,ELEMENT_EARTH],True,emoji='<:emoji_46:893471231641276496>',cooldown=5,use=MAGIE)
infinitFire = skill("Brasier","yc",TYPE_DAMAGE,500,165,AREA_DIST_5,[EXCLUSIVE,ELEMENT,ELEMENT_FIRE],True,emoji='<:emoji_44:893471208065101924>',cooldown=5,area=AREA_LINE_3,use=MAGIE)
storm = skill("Ouragan","yb",TYPE_DAMAGE,500,165,AREA_CIRCLE_2,[EXCLUSIVE,ELEMENT,ELEMENT_AIR],True,emoji='<:emoji_44:893471179023732809>',cooldown=5,area=AREA_CIRCLE_2,use=MAGIE)
...
swordDance = skill("Danse Tranchante","xy",TYPE_DAMAGE,350,power=120,use=STRENGTH,emoji='<:sworddance:894544710952173609>',cooldown=5,area=AREA_CIRCLE_2,range=AREA_MONO)
...
highkick = skill("HighKick","wu",TYPE_DAMAGE,350,power=120,range=AREA_CIRCLE_1,emoji='<:highKick:1122785057531236463>',cooldown=7,damageOnArmor=1.35)
multishot = skill("Tir Multiple","wt",TYPE_DAMAGE,350,power=75,range=AREA_DIST_4,emoji='<:name:894544834780622868>',cooldown=5,area=AREA_CONE_2)
bleedingArrow = skill("Flèche Hémorragique","ws",TYPE_DAMAGE,350,70,AREA_DIST_5,effects='mx',emoji='<:bleedingarrow:894544820071178292>',cooldown=3,effPowerPurcent=120)
bleedingDague = skill("Dague Hémorragique","wr",TYPE_DAMAGE,350,80,AREA_CIRCLE_2,effects='mx',emoji='<:bleedingdague:894552391444234242>',cooldown=3,effPowerPurcent=120)
...
bleedingTrap = skill("Lacération","vp",TYPE_INDIRECT_DAMAGE,500,effects=bleeding.id,cooldown=5,area=AREA_CIRCLE_1,use=STRENGTH,message="{0} place et déclanche un {1} autour de {2} :",emoji='<:lacerTrap:900076484230774807>',effPowerPurcent=55)
# vn already taken (Prévention)
convertEff = effect("Convertion","nk",power=50,type=TYPE_BOOST,stat=INTELLIGENCE,description="Lorsque le porteur de l'effet inflige des dégâts directs, une partie de ces dégâts lui sont rendue en Armure\nL'Intelligence du lanceur de la compétence influ sur le pourcentage de convertion",emoji=uniqueEmoji('<:convertion:900311843938115614>'))
...
vampirismeEff = effect("Vampirisme","no",power=25,type=TYPE_INDIRECT_HEAL,stat=CHARISMA,stackable=True,description="Accorde au porteur un certain pourcentage de vol de vie, augmenté par les statistiques de l'entité à l'origine de l'effet",emoji=sameSpeciesEmoji('<:vampireB:900313575913062442>','<:vampireR:900313598130282496>'))
...
focal = skill("Focalisation","vi",TYPE_INDIRECT_DAMAGE,1000,range=AREA_CIRCLE_3,cooldown=7,effects=estial.id,effPowerPurcent=225,area=AREA_CIRCLE_1,effectOnSelf=estial.id,emoji='<:focal:925204877683085332>',shareCooldown=True,use=MAGIE,description="Octrois un puissant effet __Poison d'Estialba__ aux ennemis dans la zone ciblé, avec un contre-coup",selfEffPurcent=75)
...
timeSpEff = effect("Rembobinage","nigHotMF",CHARISMA,power=35,type=TYPE_INDIRECT_HEAL,trigger=TRIGGER_START_OF_TURN,turnInit=3,emoji=uniqueEmoji('<:rlb:907687694476378112>'),stackable=True,description='Un effet de soin sur la durée qui se déclanche en début de tour')
...
strengthOfWill = skill("Force de volonté","feliUltLaunch",TYPE_DAMAGE,0,power=220,cooldown=7,ultimate=True,emoji='<:feliSlash:916208942974132245>',tpCac=True,armorConvert=80,description="Après un tour de chargement, inflige de lours dégâts à l'ennemi ciblé et en convertie une partie en armure")
...
geoConBoost = effect("Géo-Controlé","geocontroled",INTELLIGENCE,20,15,15,10,10,15,20,2,0,0,description="Une grande force vous envahis",emoji=sameSpeciesEmoji("<:geoContB:918422778414256158>","<:geoContR:918422792649723914>"))
...
geoConCast.id, geoConCast.effects, geoConCast.effectOnSelf = "sx",[None],geoConCastEff
...
rosesThornEff = effect("Epines","rosesMagEff2",CHARISMA,power=35,turnInit=3,lvl=5,trigger=TRIGGER_AFTER_DAMAGE,type=TYPE_INDIRECT_DAMAGE,onDeclancher=True,emoji=sameSpeciesEmoji("<:epineB:917665759684079626>","<:epineR:917665743649275904>"))
rosesMagicEff = effect("Pétales","rosesMagEff",CHARISMA,endurance=5,resistance=3,turnInit=3,emoji=sameSpeciesEmoji("<:rosesAreBlue:917665779770613761>","<:rosesAreRed:917665805557198878>"))
roses = skill("Roses","su",TYPE_BOOST,500,use=CHARISMA,effects=[rosesMagicEff,rosesThornEff],cooldown=5,description="Augmente les statistiques défensives de l'allié ciblé et inflige des dégâts indirects aux ennemis qui l'attaque",emoji='<:roses:932612186130493450>')
...
focus = skill("Focus","sa",TYPE_INDIRECT_DAMAGE,1000,range=AREA_CIRCLE_3,cooldown=7,effects=bleeding.id,effPowerPurcent=225,selfEffPurcent=85,effectOnSelf=bleeding.id,shareCooldown=True,use=STRENGTH,description="Applique un puissant effet __Hémorragie__ à la cible, avec un contre coup",emoji='<:focus:925204899514417182>')
poisonusPuit = skill("Rayons Féériques Triples","zzz",TYPE_DAMAGE,500,50,area=AREA_CIRCLE_1,use=MAGIE,effects=[estial.id],repetition=3,cooldown=7,emoji='<a:mpp:925181294906843136>',effPowerPurcent=85,useActionStats=ACT_INDIRECT)
bleedingPuit = skill("Pièges à ressorts","zzy",TYPE_DAMAGE,500,50,area=AREA_CIRCLE_1,effects=[bleeding.id],repetition=3,cooldown=7,emoji='<a:par:925183389252874261>',effPowerPurcent=60,useActionStats=ACT_INDIRECT)
...
lohicaUltLauch = skill("Brûme empoisonée","suz",TYPE_INDIRECT_DAMAGE,750,area=AREA_CONE_3,use=MAGIE,tpCac=True,effects=estial.id,cooldown=7,ultimate=True,emoji='<:brume:1058843224933933096>',description="Téléporte à côté de la cible et empoisonne les alentours",effPowerPurcent=150)
...
toMelee = skill('Corps à corps','suv',TYPE_DAMAGE,350,30,use=HARMONIE,tpCac=True,cooldown=3,emoji='<:cac:932765903102291999>',replay=True,range=AREA_CIRCLE_3,description="Vous téléporte au corps à corps de l'ennemi ciblé, lui inflige des dégâts et vous permet de rejouer votre tour")
toDistance = skill('Déplacement',toMelee.id,TYPE_DAMAGE,350,30,AREA_INLINE_2,use=HARMONIE,cooldown=3,jumpBack=2,emoji='<:dep:932765889017839636>',replay=True,description="Inflige des dégâts à l'ennemi ciblé, recule de deux cases et vous permet de rejouer votre tour")
...
intraEff = effect("Vulnérabilité Indirecte","traqnardEff",inkResistance=-10,turnInit=3,type=TYPE_MALUS,emoji=sameSpeciesEmoji("<:vulneB:1029816126353444894>","<:vulneR:1029816158540546120>"),description="Augmente les dégâts indirects reçus par le porteur")
intaveneuse = skill("Intraveineuse",'sqz',TYPE_INDIRECT_DAMAGE,500,effects=[estial.id,intraEff],effPowerPurcent=150,cooldown=5,emoji='<:intraveineuse:963390544770367488>',use=MAGIE)
...
cwFocus = skill("Incarnatus Puff","sqw",TYPE_INDIRECT_DAMAGE,500,cooldown=focal.cooldown-2,effects=coroWind,effPowerPurcent=250,effectOnSelf=coroWind,description="Donne un puissant effet {effIcon} __{effName}__  à l'ennemi ciblé, mais en subissez un également",emoji='<:IncPuf:968327189508542484>',jaugeEff=bloodJauge,minJaugeValue=20,maxJaugeValue=35)
propaUltLaunch = skill("Propagation Avancée","sqv",TYPE_INDIRECT_DAMAGE,750,range=AREA_MONO,emoji='<:probagUlt:1058851761680551987>',area=AREA_ALL_ENEMIES,ultimate=True,cooldown=7,effects=estial.id,effPowerPurcent=85,description="Après un tour de chargement, inflige <:est:884223390804766740> __Poison d'Estialba__ à tous les ennemis avec **35%** de sa puissance",use=MAGIE)
...
kralamSkillEff1 = effect("No no, don't touch estial.id there","squaez",trigger=TRIGGER_AFTER_DAMAGE,callOnTrigger=kralamSkillEff2,lvl=1,emoji=uniqueEmoji('<a:FranziskaNo:800833215106383883>'),type=TYPE_BOOST,resistance=5)
...
lanceToxEff = effect("Toxines","toxi",STRENGTH,power=20,emoji='<:lanceTox:971789592145580103>',stackable=True,turnInit=5,lvl=5,trigger=TRIGGER_START_OF_TURN,type=TYPE_INDIRECT_DAMAGE)
...
roseeHealEff = effect("Rosée matinale","roseedumatin",CHARISMA,agility=5,intelligence=3,charisma=3,type=TYPE_BOOST,turnInit=3,emoji=petalisEff.emoji)
roseeHeal = skill("Rosée matinale","sps",TYPE_BOOST,500,range=AREA_MONO,area=AREA_CIRCLE_3,cooldown=3,emoji='<:rosee:1119132836709007470>',effects=roseeHealEff,effectAroundCaster=[TYPE_HEAL,AREA_CIRCLE_2,35],description="Augmente l'agilité, le charisme et l'intelligence des alliés à portée tout en les soignants un peu")
...
krasisEff2 = effect("Krasis","krasisEff",INTELLIGENCE,inkResistance=10, turnInit=krasisEff.turnInit)
...
uberJauge.jaugeValue = jaugeValue(
    ...
    conds=[
        jaugeConds(INC_DEAL_HEALING,35/100)
    ]
)
...
astrodynEff = effect("Astrodynamie","astrodyn",CHARISMA,strength=5,magie=5,agility=3,precision=3,emoji='<:astro:986614953047912499>',description="Augmente les statistiques offensives du porteur")
...
cercleConEff.power, cercleConEff.stat = 4, INTELLIGENCE
...
arsenic = effect("Arsenic","arsenic",STRENGTH,power=35,turnInit=3,lvl=3,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_START_OF_TURN,stackable=True,emoji='<:arsenic:995361025773817916>')
acidRain1 = skill("Pluie Acide",getAutoId(tripleAttaque.id,True),TYPE_INDIRECT_DAMAGE,750,range=AREA_MONO,area=AREA_ALL_ENEMIES,emoji='<:acidRain:995360946241417388>',minTargetRequired=3,effects=arsenic,effPowerPurcent=100,ultimate=True,cooldown=propaUltLaunch.cooldown,description="Après un tour de chargement, inflige __Arsenic__ à tous les ennemis")
...
coroMissile1 = skill("Missile corrosif",getAutoId(coroRocket.id,True),TYPE_INDIRECT_DAMAGE,750,effPowerPurcent=165,effects=arsenic,area=AREA_CONE_3,minTargetRequired=2,cooldown=7,emoji='<:coroMissile:995361003581739018>')
...
quadFleau = skill("Quadruple Fléau",getAutoId(coroShot.id,True),TYPE_INDIRECT_DAMAGE,750,effects=[estial.id,coroWind,bleeding.id,arsenic],effPowerPurcent=65,cooldown=7,condition=[EXCLUSIVE,ELEMENT,ELEMENT_DARKNESS], emoji='<:quaFleau:1016796997887459370>')
...
sillageEff = effect("Sillage","sillageEff",MAGIE,power=35,area=AREA_CIRCLE_2,trigger=TRIGGER_END_OF_TURN,turnInit=3,lvl=3,emoji='<:sill:999763058526269510>',type=TYPE_INDIRECT_DAMAGE)
...
casseMontEff = effect("Casse Montagne","cassMont",MAGIE,power=50,lvl=5,turnInit=3,emoji="<:cassMont:999763013722701944>",description="Lorsque le porteur effectue une attaque, inflige des dégâts supplémentaires à la cible principale si cette dernière est un ennemi")
...
offrRec = skill("Offrande de Réconfort",getAutoId(finalClassique.id,True),TYPE_HEAL,500,power=int(cure.power*1),jaugeEff=lysJauge,minJaugeValue=30,description="Soigne l'allié ciblé. Cette compétence n'a pas de délais de récupération mais consomme une partie de votre {0} __{1}__ à chaque utilisation".format(lysJauge.emoji[0][0],lysJauge.name),emoji='<:recof:1000144740651634748>')
offrRav = skill("Offrande de Ravissement",getAutoId(offrRec.id,True),TYPE_HEAL,500,power=int(offrRec.power*0.65),jaugeEff=lysJauge,minJaugeValue=30,description="Vous soigne ainsi que les alliés autour de vous. Cette compétence n'a pas de délais de récupération mais consomme une partie de votre {0} __{1}__ à chaque utilisation".format(lysJauge.emoji[0][0],lysJauge.name),area=AREA_CIRCLE_4,range=AREA_MONO,emoji='<:ravis:1000144722427392010>')
confiteorReady = effect("Confiteor préparé","confiteorReady",turnInit=5,emoji='<:confiteor:1003026495050096731>')
confiteor = skill("Confiteor",getAutoId(offrRav.id,True),TYPE_DAMAGE,0,150,area=AREA_CIRCLE_1,emoji='<:confiteor:1003026495050096731>',needEffect=[confiteorReady],cooldown=5,condition=[EXCLUSIVE,ASPIRATION,ENCHANTEUR],use=MAGIE,range=AREA_CIRCLE_3,effectAroundCaster=[TYPE_HEAL,AREA_MONO,50])
holyCircleReady = effect("Cercle Sacré préparé","holyCircleReady",turnInit=5,emoji='<:cerSacre:1003026472262451300>')
cercleSacre = skill("Cercle Sacré",confiteor.id,TYPE_DAMAGE,0,135,area=AREA_CIRCLE_2,range=AREA_MONO,emoji='<:cerSacre:1003026472262451300>',needEffect=[holyCircleReady],effectOnSelf=confiteorReady,condition=[EXCLUSIVE,ASPIRATION,ENCHANTEUR],use=MAGIE,effectAroundCaster=[TYPE_HEAL,AREA_MONO,50])
holySpiritReady = effect("Esprit Saint préparé","holySpiritReady",turnInit=5,emoji='<:espSaint:1003026446706544733>')
espritSaint = skill("Esprit Saint",confiteor.id,TYPE_DAMAGE,0,100,emoji='<:espSaint:1003026446706544733>',needEffect=[holySpiritReady],effectOnSelf=holyCircleReady,condition=[EXCLUSIVE,ASPIRATION,ENCHANTEUR],use=MAGIE,range=AREA_CIRCLE_3,effectAroundCaster=[TYPE_HEAL,AREA_MONO,50])
requiescat = skill("Requiescat",confiteor.id,TYPE_DAMAGE,0,50,emoji='<:requiescat:1003026420798328974>',effectOnSelf=holySpiritReady,description="Inflige des dégâts à l'ennemi ciblé et vous permet de commencer le Combot Confiteor",condition=[EXCLUSIVE,ASPIRATION,ENCHANTEUR],use=MAGIE,range=AREA_CIRCLE_1,effectAroundCaster=[TYPE_HEAL,AREA_MONO,35],rejectEffect=[confiteorReady,holyCircleReady,holySpiritReady],replay=True)
...
fanAscend = skill("Eventails Ascendants",fanRevol.id,TYPE_DAMAGE,power=120,emoji='<:ascFan:1003308417034768444>',effectOnSelf=fanRevolReady,needEffect=[fanAscendReady],effectAroundCaster=[TYPE_BOOST,AREA_DONUT_2,fanAscendEff])
...
fanReverse = skill("Eventail Virvoltant Inversé",fanRevol.id,TYPE_DAMAGE,power=110,area=AREA_ARC_3,emoji='<:revFlyingFan:1003309639280111647>',needEffect=[fanReverseReady],effectOnSelf=fanAscendReady)
fanVir = skill("Eventail Virvoltant",fanRevol.id,TYPE_DAMAGE,power=100,area=AREA_ARC_2,emoji='<:flyingFan:1003309655444963408>',effectOnSelf=fanReverseReady)
...
ultraSignalEffect = effect("Ciblé - Ultra Signal","ultraTargeted",STRENGTH,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_ON_REMOVE,area=AREA_CIRCLE_1, power=70, emoji=sameSpeciesEmoji("<:ultraB3:1105108367023816857>","<:ultraR3:1105108390658707507>"))
...
mendRegen = effect("Remède Sylvestre","mendRegen",CHARISMA,power=18,turnInit=3,type=TYPE_INDIRECT_HEAL,trigger=TRIGGER_START_OF_TURN,emoji="<:mend:1023609835981389854>")
...
assasinate = skill("Assassinat",getAutoId(lifeSeed.id,True),TYPE_DAMAGE,750,power=150,range=AREA_CIRCLE_2,tpCac=True,effects=assasinateEff,cooldown=7,effBeforePow=True,ultimate=True,erosion=65,emoji='<:assasinate:1030105420552994837>',description="Augmente les dégâts reçus par la cible et lui inflige de gros dégâts tout en réduisant ses PV Maximums")
...
retourneImpactant = skill("Retourné Impactant",getAutoId(piedVolt.id,True),TYPE_DAMAGE,500,cooldown=5,power=95,range=AREA_CIRCLE_3,effectOnSelf=retImpEff,description="Inflige des dégâts à l'ennemi ciblé, vous fait reculer d'une case et vous octroi une armure",jumpBack=1,emoji='<:retPercu:1034210776703062057>')
...
tripleChoc.id, tripleChoc.power, tripleChoc.area, tripleChoc.jumpBack, tripleChoc.maxPower, tripleChoc.replay, tripleChoc.effectOnSelf, tripleChoc.emoji, tripleChoc.minJaugeValue, tripleChoc.maxJaugeValue ,affSkillMsg = tripleChoc_2.id.replace("_2",""), 60, AREA_ARC_1, 0, 100, True, tripleChoc_1_c, "<:chainChoc1:1058339074563379240>", 25, 50, True
...
leadShotEff = effect("Balle de Plomb","leadShotEff",STRENGTH,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_START_OF_TURN,power=20,turnInit=3,emoji='<:leadShot:1065676062874419222>')
...
heatedSlugShot = skill("Tir de Balle Chauffé",heatedCleanShot.id,TYPE_DAMAGE,500,power=105,effectOnSelf=heatedCleanShotReady,needEffect=heatedSlugShotReady,emoji='<:cleanShotCombo2:1065647381208641556>',description="Inflige des dégâts à l'ennemi ciblé et vous permet d'utiliser {0} {1}".format(heatedCleanShot.emoji,heatedCleanShot.name))
heatedSlitShot = skill("Tir Scyndé Chauffé",heatedCleanShot.id,TYPE_DAMAGE,power=95,effectOnSelf=heatedSlugShotReady,rejectEffect=[heatedSlugShotReady,heatedCleanShotReady],emoji='<:cleanShotCombo1:1065647347113148436>',description="Inflige des dégâts à l'ennemi ciblé et vous permet d'utiliser {0} {1}".format(heatedSlugShot.emoji,heatedSlugShot.name))
...
healingSacrificeEff = effect("Régénération Sacrificielle","healingSacrifice",CHARISMA,power=80,trigger=TRIGGER_START_OF_TURN,type=TYPE_INDIRECT_HEAL,emoji='<:AltyRegen:994748551932428319>')
...
infDraw = skill("Dessin Infecteux",getAutoId(funDraw.id,True),types=TYPE_DAMAGE,price=500,power=100,cooldown=5,use=MAGIE,useActionStats=ACT_INDIRECT,emoji='<:marqInf:1073978855766892605>',description="Inflige des dégâts à la cible. Rénitialise la puissance et la durée des effets {0} __{1}__, {2} __{3}__ et {4} __{5}__ présents sur la cible".format(infection.emoji[0][0],infection.name,epidemicEff.emoji[0][0],epidemicEff.name,intoxEff.emoji[0][0],intoxEff.name))
moramMortemEff = effect("Moram Mortem","moramMortem",INTELLIGENCE,type=TYPE_ARMOR,overhealth=50,turnInit=3,trigger=TRIGGER_DAMAGE,emoji='<:moramMortis:1084865158884630528>',stackable=True)
extensasVita = effect("Extensas Vita","extensasVita",INTELLIGENCE,type=TYPE_ARMOR,overhealth=moramMortemEff.overhealth,turnInit=moramMortemEff.turnInit,trigger=TRIGGER_DAMAGE,emoji='<:extensaVita:1084865373733650574>',stackable=True)
...
lifeLight = skill("Lux Vitae",getAutoId(lifeFlame.id,True),price=750,emoji='<:lifeLight:1084263898926035024>',use=MAGIE,types=TYPE_DAMAGE,power=int(galvanisation.power*1.15),effectAroundCaster=galvanisation.effectAroundCaster,effPowerPurcent=galvanisation.effPowerPurcent,condition=[EXCLUSIVE,ASPIRATION,MAGE],description="Inflige des dégâts à l'ennemi ciblé et octroi un effet de régénération à vous et vos alliés proches")
...
decompositionEff = effect("Décompositionis","decompositionEff",MAGIE,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_END_OF_TURN,power=40,turnInit=3,emoji='<:dcomposition:1084872910314950656>')
...
synastieEff = effect("Synastie","synastieEff",CHARISMA,strength=7, magie=7,turnInit=3,emoji='<:muscBat:1028695550645784576>')
...
fracas = skill("Talon Fracassant",getAutoId(synastie.id,True),TYPE_DAMAGE,use=STRENGTH,price=500,power=115,cooldown=7,effects=fracasEff,range=AREA_CIRCLE_2,emoji='<:talonFracassant:1088958995030605834>',description="Inflige des dégâts à l'ennemi ciblé et réduit l'armure qu'il reçoit durant 3 tours. Dégâts augmentés sur l'armure",damageOnArmor=1.2)
...
plongeFracas = skill("Salto Fracassant",getAutoId(revolutionFracas.id,True),TYPE_DAMAGE,emoji='<:plongee:1088958906178474076>',range=AREA_CIRCLE_2,power=100,effects=[plongeFracasEff,revolutionFracas],cooldown=7,description="Inflige des dégâts à l'ennemi ciblé et réduit l'armure qu'il reçoit durant 3 tours. Inflige également des dégâts indirects aux ennemis proches",price=500)
holyVoiceEff1 = effect("Voix divine","divineVoice",CHARISMA,strength=10,magie=10,endurance=5,emoji='<:holyVoice:1085991767834370108>',turnInit=3)
...
weakPointEff1 = effect("Point Faible révélé","weakPointRevealed",stat=PURCENTAGE,resistance=-20,type=TYPE_MALUS)
...
bloodMoonEff.power = bloodMoonEff2.power = 5.25
bloodMoon = skill("Lever de la Lune de Sang",getAutoId(ripping.id,True),TYPE_BOOST,price=750,cooldown=5,area=AREA_CIRCLE_2,effects=[bloodMoonEff,bloodMoonEff2],emoji='<:BloodMoon:1094026774448443512>',description="Augmente les dégâts infligés par les alliés ciblés et leur octroi un pourcentage de vol de vie",condition=[EXCLUSIVE,ELEMENT,ELEMENT_SPACE])
blueMoonEff2 = copy.deepcopy(convertEff)
blueMoonEff2.stat, blueMoonEff2.power = bloodMoonEff2.stat, bloodMoonEff2.power
blueMoon = skill("Lever de la Lune Bleu",getAutoId(bloodMoon.id,True),TYPE_BOOST,price=750,cooldown=5,area=AREA_CIRCLE_2,effects=[bloodMoonEff,blueMoonEff2],emoji='<:blueMoon:1094026561784643604>',description="Augmente les dégâts infligés par les alliés ciblés et convertie une partie des dégâts qu'ils infligent en armure",condition=[EXCLUSIVE,ELEMENT,ELEMENT_SPACE])
newMoonEff, newMoonEff2 = copy.deepcopy(dmgDown), copy.deepcopy(armorGetMalus)
newMoonEff.stat, newMoonEff.power, newMoonEff2.power = INTELLIGENCE, 4, 20
newMoon = skill("Lever de la Nouvelle Lune",getAutoId(blueMoon.id,True),TYPE_MALUS,price=750,cooldown=5,area=AREA_CIRCLE_2,effects=[newMoonEff,newMoonEff2],emoji='<:newMoon:1094026598690324573>',description="Réduits les dégâts infligés par les ennemis ciblés ainsi que l'armure qu'ils reçoivent",condition=[EXCLUSIVE,ELEMENT,ELEMENT_SPACE])
fullMoon = skill("Lever de la Pleine Lune",getAutoId(newMoon.id,True),TYPE_MALUS,price=750,cooldown=5,area=AREA_CIRCLE_2,effects=[newMoonEff,incur[2]],emoji='<:newMoon:1094026598690324573>',description="Réduits les dégâts infligés par les ennemis ciblés ainsi que les soins qu'ils reçoivent",condition=[EXCLUSIVE,ELEMENT,ELEMENT_SPACE])
...
moonLight = skill("Clair de Lune",getAutoId(fullMoon.id,True),TYPE_BOOST,price=500,cooldown=5,effects=[moonLightEff1,moonLightEff2],area=AREA_CIRCLE_2,emoji=moonLightEff2.emoji[0][0],description="Réduit les dégâts subis par les alliés ciblés et leur octroi une légère armure")
lacerEff = effect("Lacération","lacerationEff",stat=PURCENTAGE,resistance=-5,type=TYPE_MALUS,stackable=True,emoji='<:laceration:1100428518401245194>')
exploStrikeEff = effect("Frappe Explosive","exploStrike",stat=STRENGTH,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_INSTANT,area=AREA_CIRCLE_1,power=25,emoji='<:exploShotEff:1049704001257619476>')
pentastrike5 = skill("PentaStrike Explosive",getAutoId(moonLight.id,True),TYPE_DAMAGE,price=750,ultimate=True,affSkillMsg=False,range=AREA_CIRCLE_1,effects=[lacerEff,exploStrikeEff],effPowerPurcent=150,power=50,emoji='<:kPentaStrikeFinal:1101004392506855515>',cooldown=7,description="Après un tour de chargement, inflige 5 coups à l'ennemi ciblé. Chaque attaque réduit sa résistance et inflige des dégâts indirects suplémentaires à la cible et aux ennemis proches")
pentastrike5_c = effect("Enchaînement - {replicaName}","pentaStrike5",silent=True,replique=pentastrike5)
pentastrike4 = copy.deepcopy(pentastrike5)
pentastrike4.power, pentastrike4.effectOnSelf, pentastrike4.emoji, pentastrike4.replay = 30, pentastrike5_c, "<:kPentaStrike:1101004369727602688>", True
pentastrike4_c = effect("Enchaînement - {replicaName}","pentaStrike4",silent=True,replique=pentastrike4)
pentastrike3 = copy.deepcopy(pentastrike4)
pentastrike3.power, pentastrike3.effectOnSelf = 30, pentastrike4_c
pentastrike3_c = effect("Enchaînement - {replicaName}","pentaStrike3",silent=True,replique=pentastrike3)
pentastrike2 = copy.deepcopy(pentastrike3)
pentastrike2.power, pentastrike2.effectOnSelf = 20, pentastrike3_c
pentastrike2_c = effect("Enchaînement - {replicaName}","pentaStrike2",silent=True,replique=pentastrike2)
pentastrike1 = copy.deepcopy(pentastrike2)
pentastrike1.power, pentastrike1.effectOnSelf, pentastrike1.affSkillMsg = 20, pentastrike2_c, True
...
kStrikeEff = effect("Saignement","bleedingKEff",STRENGTH,type=TYPE_INDIRECT_DAMAGE,power=25,turnInit=3,trigger=TRIGGER_START_OF_TURN,lifeSteal=250,emoji='<:bleed:1083761078770618460>')
kStrike = skill("Frappe Sanglante",getAutoId(gravPulse.id,True),TYPE_DAMAGE,range=AREA_CIRCLE_1,power=95,effects=kStrikeEff,cooldown=5,condition=[EXCLUSIVE,ASPIRATION,BERSERK],emoji='<:kStrike:1100644659241422891>',price=500,description="Inflige des dégâts à l'ennemi ciblé ainsi qu'un effet de dégâts période vous soignant en fonction des dégâts qu'il inflige")

gwenPenta5 = skill("PentaStrike Armurée",getAutoId(kStrike.id,True),TYPE_DAMAGE,price=750,range=AREA_CIRCLE_1,use=INTELLIGENCE,affSkillMsg=False,damageOnArmor=1.2,power=pentastrike5.power,effects=lacerEff,cooldown=pentastrike5.cooldown,ultimate=True,emoji='<:gPentaStrineFinal:1100778233831641118>',armorConvert=70,accuracy=150,aoeArmorConvert=50,description="Après un tour de chargement, inflige 5 coups à l'ennemi ciblé. Chaque attaque réduit sa résistance et octroi à vous et vos alliés proches une armure en fonction des dégâts infligés")
gwenPenta5_c = effect("Enchaînement - {replicaName}","pentaStrike5",silent=True,replique=gwenPenta5)
gwenPenta4 = copy.deepcopy(gwenPenta5)
gwenPenta4.power, gwenPenta4.effectOnSelf, gwenPenta4.emoji, gwenPenta4.replay, gwenPenta4.armorConvert, gwenPenta4.aoeArmorConvert = pentastrike4.power, gwenPenta5_c, "<:gPentaStrike:1100778209450139688>", True, 50, 35
gwenPenta4_c = effect("Enchaînement - {replicaName}","pentaStrike4",silent=True,replique=gwenPenta4)
gwenPenta3 = copy.deepcopy(gwenPenta4)
gwenPenta3.power, gwenPenta3.effectOnSelf = pentastrike3.power, gwenPenta4_c
gwenPenta3_c = effect("Enchaînement - {replicaName}","pentaStrike3",silent=True,replique=gwenPenta3)
gwenPenta2 = copy.deepcopy(gwenPenta3)
gwenPenta2.power, gwenPenta2.effectOnSelf = pentastrike3.power, gwenPenta3_c
gwenPenta2_c = effect("Enchaînement - {replicaName}","pentaStrike2",silent=True,replique=gwenPenta2)
gwenPenta1 = copy.deepcopy(gwenPenta2)
gwenPenta1.power, gwenPenta1.effectOnSelf, gwenPenta1.affSkillMsg = pentastrike1.power, gwenPenta2_c, True
...
gwenCharge = skill("Taillade Sautée",getAutoId(gwenPenta5.id,True),TYPE_DAMAGE,power=85,price=500,range=AREA_CIRCLE_4,tpCac=True,use=INTELLIGENCE,armorConvert=125,aoeArmorConvert=75,cooldown=5,emoji="<:gStrike:1100681148876718120>",description="Saute sur l'ennemi ciblé, lui inflige des dégâts et octroi à vous et vos alliés proches une armure en fonction des dégâts infligés")
...
altyPenta4 = copy.deepcopy(altyPenta5)
altyPenta4.power, altyPenta4.effectOnSelf, altyPenta4.emoji, altyPenta4.replay, altyPenta4.lifeSteal, altyPenta4.aoeLifeSteal = pentastrike4.power, altyPenta5_c,'<:aPentaStrike:1101009306121416734>', True, 50, 35
altyPenta4_c = effect("Enchaînement - {replicaName}","pentaStrike4",silent=True,replique=altyPenta4)
altyPenta3 = copy.deepcopy(altyPenta4)
altyPenta3.power, altyPenta3.effectOnSelf = pentastrike3.power, altyPenta4_c
altyPenta3_c = effect("Enchaînement - {replicaName}","pentaStrike3",silent=True,replique=altyPenta3)
altyPenta2 = copy.deepcopy(altyPenta3)
altyPenta2.power, altyPenta2.effectOnSelf = pentastrike3.power, altyPenta3_c
altyPenta2_c = effect("Enchaînement - {replicaName}","pentaStrike2",silent=True,replique=altyPenta2)
altyPenta1 = copy.deepcopy(altyPenta2)
altyPenta1.power, altyPenta1.effectOnSelf, altyPenta1.affSkillMsg = pentastrike1.power, altyPenta2_c, True
