...
async def fight(bot : discord.Client ,team1 : list, team2 : list ,ctx : SlashContext,guild,auto = True,contexte=[],octogone=False,slash=False):
    ...
    class entity:
        ...
        def __init__(self,identifiant : int, perso : Union[char,tmpAllie,octarien], team : int, player=True, auto=True, danger=100,summoner=None):
            ...
            baseStats[ENDURANCE] += int(25*valueToBoost)
            baseStats[RESISTANCE] += int(15*valueToBoost)
            baseStats[PERCING] += int(10*valueToBoost)
                                ...
                                if use != None:
                                    for statBoost in self.effect:
                                        ...
                                        if effStat != 0 and statBoost.caster != self: # If the stat was boosted by someone esle
                                            ...
                                            tempDmg = round(round(value * (used+100)/100 * (1-(min(95,entityInArea.resistance*(1-self.percing/100))/100)) * dangerMul)*multiplicateur*ailill/100)
            ...
            for cmpt in range(len(allReadySeenID)):
                ...
                if tempLen > 1:
                    for cmpt2 in range(tempLen):
                        ultimateTemp += "__{0}__ ({1})".format(unAffected[cmpt][cmpt2][0],unAffected[cmpt][cmpt2][1])
                    ...
                    ultimateTemp += "__{0} ({1})__ n'est plus sous l'effet de __{2}__\n".format(unAffected[cmpt][0][0],unAffected[cmpt][0][1],effNames[cmpt])
        ...
        def summon(self,summon : invoc ,timeline, cell : cell, tablEntTeam : list,tablAliveInvoc : list, ignoreLimite=False):
            ...
            if tablAliveInvoc[team] < 3 or ignoreLimite:
                ...
                return tablEntTeam,tablAliveInvoc,timeline,ballerine
        ...
        def heal(self,target, icon : str, statUse : int, power : int, effName = None,danger=danger):
            ...
            if power > 0 and self.hp > 0:
                if statUse != None:
                    ...
                    healPowa = round(power * (1+(statUse-self.negativeHeal)/120+(target.endurance/1000))* self.valueBoost(target = target,heal=True)*self.getElementalBonus(target,area = AREA_MONO,type = TYPE_HEAL)*dangerBoost)
        ...
        def knockback(self, target, power:int):
            ...
            if len(posArea) > 0:
                ...
                cmpt = 1
                cellToPush = posArea[int(len(posArea) > 1)]
                while cellToPush.on == None and cmpt < len(posArea)-1:
                    cmpt += 1
                ...
                if cmpt < power:
                    ...
                    badaboum = [target]
                    if cmpt != len(posArea)-1 and posArea[cmpt].on != None:
    ...
    class fightEffect:
        ...
        def __init__(self,id,effect : classes.effect, caster : entity, on : entity, turnLeft : int, trigger : int, type : int, icon : str, value=1, effReducPurcent:Union[None,int]=None):
            ...
            if self.effect.stat != None:
                ...
                valueBoost = (1 + self.caster.char.level//10/10 - 1 + caster.valueBoost(on)) * (temp+100)/100
            else:
                valueBoost = 1
        ...
        def triggerDamage(self,value=0,icon="",declancher = 0,onArmor = 1) -> list:
            ...
            if self.type == TYPE_ARMOR and value >0:
                ...
                for a in target.cell.getEntityOnArea(area=self.effect.area,team=target.team,wanted=whoTarget,directTarget=False):
                    reduc = 1
                    if a != target:
                        reduc = max(AEOMINDAMAGE,1-target.cell.distance(a.cell)*AOEDAMAGEREDUCTION)
        ...
        def triggerStartOfTurn(self,danger,decate=True) -> str:
            ...
            if self.type == TYPE_INDIRECT_HEAL and self.on.hp > 0:              # Heal Indirect
                if self.effect.area == AREA_MONO:
                    ballerine = self.caster.heal(self.on,self.icon,self.effect.stat,self.effect.power,self.effect.name,danger)
                else:
    ...
    def add_effect(caster: entity,target: entity,effect: classes.effect,start: str = "",ignoreEndurance=False,turn=tour,danger=danger,setReplica : Union[None,entity] = None,effPowerPurcent=100):
    ...
    if team2 == []: # Génération de la team 2
        ...
        if random.randint(0,99) < 80 or leni < 4: # Vs normal team
            ...
            while len(team2) < len(team1) and not(oneVAll):
                temp = random.randint(0,len(tablOctaTemp)-1)

                if len(team2) == 2:
                    alea = copy.deepcopy(findEnnemi("Octaling"))
                else:
                    alea = tablOctaTemp[temp]
                    tablOctaTemp.remove(tablOctaTemp[temp])
    ...
    if tablEntTeam[1][0].isNpc("The Giant Enemy Spider"):           # Giant Enemy Spider's legs summoning
        ...
        for cmpt in range(8):
            ...
            tablEntTeam, tablAliveInvoc, time, ballerine = tablEntTeam[1][0].summon(toSummon,time,surrondingsCells[cmpt],tablEntTeam,tablAliveInvoc,ignoreLimite=True)
            logs += "\n"+ballerine
                ...
                if not(actTurn.stun):                   # The entity isn't stun, so he can play his turn
                    ...
                    if optionChoice == OPTION_WEAPON: #Option : Attaque
                        ...
                        if skillToUse.type in [TYPE_BOOST,TYPE_ARMOR,TYPE_INDIRECT_HEAL,TYPE_INDIRECT_REZ]:
                                ...
                                for eff in ennemi.effect:
                                    if eff.effect.id in [estal.id,bleeding.id] and eff.value > 0:
                                        for ent in ennemi.cell.getEntityOnArea(area=AREA_DONUT_2,team=actTurn.team,wanted=ENNEMIS,directTarget=False):
                                            add_effect(actTurn,ent,finalTablPropa[int(eff.effect.id == bleeding.id)][int(eff.caster!=actTurn)][eff.value-1])
                            ...
                            tablEntTeam, tablAliveInvoc, time, ballerine = actTurn.summon(findInvoc(skillToUse.invocation),time,actTurn.cell.getCellForSummon(skillToUse.range,actTurn.team,findInvoc(skillToUse.invocation),actTurn),tablEntTeam,tablAliveInvoc)
                            tempTurnMsg += ballerine+"\n"
                            logs += "\n"+ballerine
                            ...
                            if skillToUse.effect!=[None]:
                                for a in skillToUse.effect:
                                    effect = findEffect(a)
                                    tempTurnMsg += "\n"+add_effect(actTurn,ennemi,effect)
                            ...
                            if skillToUse.id == lunaSpe4.id:
                                ...
                                while lenSummon < len(team1)-1:
                                    ...
                                    tablEntTeam, tablAliveInvoc, time, ballerine = actTurn.summon(findInvoc("Conviction des Ténèbres"),time,celli,tablEntTeam,tablAliveInvoc,ignoreLimite=True)
                                    tempTurnMsg += ballerine+"\n"
                                    logs += "\n"+ballerine
                                    lenSummon += 1
        ...
        while startMsg: # Tableau des statistiques
            ...
            try:
                ...
                if stats.totalNumberShot > 0:
                    precisePurcent = round(stats.shootHited/stats.totalNumberShot*100) +"%"
                    critPurcent = round(stats.crits/stats.totalNumberShot*100) +"%"
                if stats.numberAttacked > 0:
                    dodgePurcent = round(stats.dodge/stats.numberAttacked*100) +"%"

                ballerine =[[stats.ennemiKill,stats.damageDeal,stats.indirectDamageDeal,stats.underBoost,stats.damageOnShield],[stats.allieResurected,stats.heals,stats.damageBoosted,stats.damageDogded,stats.shieldGived],[stats.survival,stats.damageRecived,stats.selfBurn,precisePurcent,dodgePurcent,critPurcent]]
                babie = [["Ennemis tués :","Dégâts infligés :","> - Dont indirects :","> - Dont Boostés :","Dégâts sur armure :"],["Alliés ressucités :","Soins effectués :","Dégâts Boostés :","Dégâts réduits :","Armure fournie :"],["Tours survécus :","Dégâts reçus :","Dégats sur soi-même :","Taux de précision : ","Taux d'esquive :","Taux de coup critique :"]]
                ...
                for a in [0,1,2]:
                    ...
                    for b in range(0,len(ballerine[a])):
                        ...
                        if ballerine[a][b] == 0:
                            temp =""
                        statMsg += f"\n{babie[a][b]} {temp}{str(ballerine[a][b])}{temp}"
