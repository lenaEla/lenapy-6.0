                        ...
                        match tmpKey:
                            ...
                            case "stuff":
                                ...
                                for tmpStuff in stuffs:
                                    if (tmpChar.have(tmpStuff) and random.randint(0,9) < 9) or tmpStuff.price == 0: tmpPull.remove(tmpStuff)
                                pulledStuff: stuff = randRep(tmpPull)
                            ...
                            case "weapon":
                                ...
                                for tmpWeapon in weapons:
                                    if (tmpChar.have(tmpWeapon) and random.randint(0,9) < 9) or tmpWeapon.price == 0: tmpPull.remove(tmpWeapon)
                                pulledStuff: weapon = randRep(tmpPull)
                                if tmpChar.have(pulledStuff): listLoot["exp"] += 25
                                else: tmpChar.weaponInventory.append(pulledStuff); listLoot["loot"] += "{0}\n".format(pulledStuff)
                            case "skill":
                                tmpPull = skills[:]
                                for tmpSkill in skills:
                                    if (tmpChar.have(tmpSkill) and random.randint(0,9) < 9) or tmpSkill.price == 0: tmpPull.remove(tmpSkill)
                                for tmpSkill in tmpPull[:]:
                                    if (tmpSkill.use in [STRENGTH,AGILITY,PRECISION,ENDURANCE] and location == 2) or (tmpSkill.use in [CHARISMA,INTELLIGENCE,MAGIE] and location == 1): tmpPull.append(tmpSkill)
                                pulledStuff: skill = randRep(tmpPull)
                                if tmpChar.have(pulledStuff): listLoot["exp"] += 25
