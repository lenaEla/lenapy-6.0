...
from classes import weapon, skill, stuff, effect, char, octarien
from typing import List, Union
from advance_gestion import cell

def getPartnerValue(user : char):
...
def dmgCalculator(caster: "entity", target: "entity", basePower:float, use:int, actionStat:int, area:int, typeDmg:int = TYPE_DAMAGE, skillPercing = 0, ignoreEff = None):
	dangerMul = 1
	if caster.team: dangerMul = caster.danger / 100
	...
	if use not in [FIXE,None,MISSING_HP,PURCENTAGE]:
		statBucket, lvlBonus, elemBonus = baseStat[use] + baseStat[actionStat], BASEDAMAGEMUL + (DMGBONUSPERLEVEL*caster.level), caster.getElementalBonus(target=target, area=area, type=typeDmg) 
		if isMelee: statBucket += int(baseStat[ENDURANCE] * MELEE_END_CONVERT/100)
		if caster.char.__class__ == octarien: lvlBonus = min(lvlBonus, BASEDAMAGEMUL + (DMGBONUSPERLEVEL * MAXLEVEL))
		dmg = basePower * max(-65,statBucket)/100 *dangerMul* lvlBonus* elemBonus
		...
		for eff in caster.effects:
			...
			if effStats[use] != 0 or (isMelee and effStats[ENDURANCE] != 0):
				...
				tmpDmg = basePower * max(-65,statBucket1)/100 * dangerMul * lvlBonus * elemBonus
		...
		caster.logs += "\n> {0} [basePower] * ({1} [AtkStats] / 100) * {2} [dangerMul] * {3} [elemBonus] * {4} [lvlBonus]  * {5} [resistFactor]".format(basePower, statBucket, dangerMul , elemBonus, lvlBonus,round(resistFactor,2))
		return dmg, dictContributor, resistedDmg
...
def indirectDmgCalculator(caster: "entity", target: "entity", basePower, use, area, useActionStat = ACT_INDIRECT):
	damage, logs = basePower, "FIXE"
	dangerMul = 1
	if caster.team: dangerMul = caster.danger / 100
	
	if use not in [None,FIXE,PURCENTAGE,MISSING_HP]:
		if use != HARMONIE: stat = caster.allStats[use] + caster.actionStats[useActionStat]
		else: stat = max(caster.allStats) + max(caster.actionStats)
...
def getHealAggro(caster,target, skillToUse : Union[skill,weapon],armor=False):
	if type(target.char) == invoc or target.hp <= 0 or target.hp >= target.maxHp:
		...
		if (skillToUse.type == TYPE_HEAL and (type(skillToUse) == skill and skillToUse.cooldown >= 7) and target.hp/target.maxHp >= 0.7) or (skillToUse.type == TYPE_INDIRECT_HEAL and target.hp/target.maxHp <= 0.35):
...
def calHealPower(ent: "entity", target: "entity", power:float, secElemMul:float, statUsed:int, usedActionStat: int) -> float:
	if ent.team: danger = 100
	else: danger = ent.danger

	vigilBonus = [1, 1.1][target.char.aspiration == VIGILANT]

	if statUsed == HARMONIE:
		statBucket = max(ent.allStats) + max(ent.actionStats)
	elif statUsed <= MAGIE:
		statBucket = ent.allStats[statUse] + ent.actionStats[usedActionStat]
	else:
		statBucket = 0

	if ent.aspiration in meleeAspi: statBucket += int(ent.endurance * MELEE_END_CONVERT/100)

	targetHealReciveBonus = 1 + min((target.endurance / END_NEEDED_10_HEAL * 10), MAX_END_10_HEAL)/100
	statMul = 1 + max(-65,statBucket)/100
	return round(power * secElemMul * statMul* targetHealReciveBonus *ent.valueBoost(target = target,heal=True) * ent.getElementalBonus(target, area = AREA_MONO,type = TYPE_HEAL) * danger/100 * vigilBonus)

def getBuffAggro(caster,target,effect:effect):
	if type(effect) != effect or (caster == target and effect.turnInit < 2):
		return 0
