...
def from_adress(adress: int):
	to_return = ctypes.cast(adress, ctypes.py_object)
...
class timeline:
	...
	def icons(self):
		...
		for icn in self.timeline:
			...
			elif icn.icon not in duplicateList and type(icn.char) not in [invoc, octarien]:
	...
	def endOfTurn(self, tablEntTeam, entDict):
...
class localVarObj:
	def __init__(self, tablEntTeamId: int, deplTablId: int, entDictId:int , logsId: int, localTimelineId: int, dangerId: int, tablAllCellsId: int, turnNumberId:int, otherVarsId: int):
		self.tablEntTeam = tablEntTeamId
		self.deplTabl = deplTablId
		self.entDict = entDictId
		self.logs = logsId
		self.localTimeline = localTimelineId
		self.danger = dangerId
		self.tablAllCells = tablAllCellsId
		self.otherVars = otherVarsId
		self.turnNumber = turnNumberId

class entity:
	"""Base class for the entities"""
	def initBaseVariables(self, localVar:localVarObj, summoner: "entity" = None):
		if localVar != None:
			self.tablEntTeam: List[List["entity"]] = from_adress(localVar.tablEntTeam)
			self.deplTabl: List[List["entity"]] = from_adress(localVar.deplTabl)
			self.entDict: dict = from_adress(localVar.entDict)
			self.logs: logObject = from_adress(localVar.logs)
			self.timeline: timeline = from_adress(localVar.localTimeline)
			self.danger: int = from_adress(localVar.danger)
			self.tablAllCells: List[cell] = from_adress(localVar.tablAllCells)
			self.turnNumber: int = from_adress(localVar.turnNumber)
			self.otherVars: dict[List[list]] = from_adress(localVar.otherVars)
		elif summoner != None:
			self.tablEntTeam: List[List["entity"]] = from_adress(id(summoner.tablEntTeam))
			self.deplTabl: List[List["entity"]] = from_adress(id(summoner.deplTabl))
			self.entDict: dict = from_adress(id(summoner.entDict))
			self.logs: logObject = from_adress(id(summoner.logs))
			self.timeline: timeline = from_adress(id(summoner.timeline))
			self.danger: int = from_adress(id(summoner.danger))
			self.tablAllCells: List[cell] = from_adress(id(summoner.tablAllCells))
			self.turnNumber: int = from_adress(id(summoner.turnNumber))
			self.otherVars: dict[List[list]] = from_adress(id(summoner.otherVars))
		else:
			raise AttributeError("No localVar passed")

	def __init__(self, identifiant : int, character : Union[char, tmpAllie, octarien, invoc, depl], team : int, localVar:localVarObj=None, player=True, auto=True, summoner=None):
		"""
			Parameters :\n
		...
		try:
			self.initBaseVariables(localVar, summoner)
		except:
			print_exc()
			raise
		self.char = character
		...
		self.effectWaitList: List["fightEffect"] = []
		...
		self.actHeal, self.actBoost, self.actShield, self.actDirect, self.actIndirect = 0,0,0,0,0
		self.ressurectable = False
		self.skills: List[skill] = self.char.skills
		self.clemBloodJauge, self.aspiSlot = None, None

		if type(self.char) != depl: # Skill Verif
			tmpList = []
			for tmpSkill in self.skills:
				if type(tmpSkill) == skill or type(findSkill(tmpSkill)) == skill: tmpList.append(tmpSkill)
			self.skills = tmpList
			...
			self.effects, self.specialVars = self.entDict[self.summoner.id].effects, self.entDict[self.summoner.id].specialVars
	...
	def init_stats(self):
		if type(self.char) not in [invoc, depl]:
			...
			for obj in [self.char.weapon,self.char.stuff[0],self.char.stuff[1],self.char.stuff[2]]:
				...
				if type(self.char) == octarien and self.team == 1: dangerMul = (100 + (20*((self.danger-100)/50)))/100
				else: dangerMul = 1
				...
				baseStats[10] += int(obj.actHeal)
				baseStats[11] += int(obj.actBoost)
				baseStats[12] += int(obj.actShield)
				baseStats[13] += int(obj.actDirect)
				baseStats[14] += int(obj.actIndirect)
			...
			if self.char.__class__ == tmpAllie and self.team == 1 and not(octogone):
		...
		if self.danger > 100 and type(self.char) == octarien:
			add = int(self.danger/135 * 20)
		...
		if type(self.char) in [octarien,tmpAllie] and self.team: self.maxHp = round(self.maxHp * self.danger / 100)
	...
	def move(self, cellToMove:Union[cell, None]=None, x:Union[int,None]=None, y:Union[int,None]=None):
		...
		actCell = self.cell
		if cellToMove == None: destination = findCell(x,y, self.tablAllCells)
		else: destination = findCell(cellToMove.x,cellToMove.y, self.tablAllCells)
		self.cell = destination
		destination.on = self
		if actCell != None: actCell.on = None

		for tmpDepl in self.deplTabl:
			if tmpDepl[0].char.trap and self.cell in tmpDepl[2]:
				...
				if tmpDepl[0].leftTurnAlive <= 0:
					...
					if len(tmpDepl[0].ownEffect) == 0:
						try: self.deplTabl.remove(tmpDepl); tmpDepl[1].depl = None
						except: print_exc()

		for eff in self.effects:
			if eff.effects.trigger == TRIGGER_ON_MOVE: toReturn += eff.triggerStartOfTurn()
			...
			for a in self.effects:
				if a.effects.stackable and a.effects.id in allReadySeenId and a.effects.id not in effListShowPower:
					...
					if a.type == TYPE_ARMOR: res += f"{a.icon} {name} ({a.value} PAr)\n"
						...
						res += f"{a.icon} {name} ({a.turnLeft} self.turnNumber{pluriel})\n"
	...
	def recalculate(self):
		...
		for eff in self.effects:
			...
			elif (eff.effects.id == lastDitchEffortEff.id and (self.turnNumber >= 17 or self.hp/self.maxHp <= 0.25)) or (eff.effects.id == openingGambitEff.id and (self.turnNumber <= 1 or self.hp/self.maxHp >= 0.9)):
	...
	def refreshEffects(self):
		...
		for eff in self.effectWaitList[:]:
			if type(eff) != fightEffect: raise AttributeError("Except fightEffect, but get {0} insted".format(type(a)))
			self.effects.append(eff)
			eff.caster.ownEffect.append({self:eff.id})
			self.effectWaitList.remove(eff)
		self.recalculate()

	def indirectAttack(self,target: "entity", value=0, icon="", ignoreImmunity=False, name='', canCrit=True, redirected=False) -> str:
		popopo, hasCrit = "", ""
		if type(value) == tuple: value = value[0]
		if canCrit and random.randint(0,99) < self.intelligence/200*25 * (1-(target.intelligence/100*10)/100):
			value, hasCrit = value * [1.15,1.25][self.char.aspiration in [SORCELER,ATTENTIF]], " !"
		...
		if target.hp > 0 and not(target.immune):
			...
			if value > 0:
				...
				if target.hp <= 0 and len(self.tablEntTeam[target.team]) == 1 and self.isNpc("Lia Ex"):
				...
				for team in [0,1]:
					for jaugeEnt, jaugeEff in self.otherVars["teamJaugeDict"][team].items():
				...
				try:
					for conds in self.otherVars["teamJaugeDict"][self.team][[self,self.summoner][type(self.char) in [invoc,depl]]].effects.jaugeValue.conds:
						if conds.type == INC_DEAL_DAMAGE:
							self.otherVars["teamJaugeDict"][self.team][[self,self.summoner][type(self.char) in [invoc,depl]]].value = min(self.otherVars["teamJaugeDict"][self.team][[self,self.summoner][type(self.char) in [invoc,depl]]].value+((conds.value*value/target.maxHp*100)*([1,5][target.char.standAlone])),100)
	...
	def death(self, killer = 0, trueDeath=False) -> str:
		...
		if not(trueDeath):
			for eff in self.effects:
				if eff.effects.id in [holmgangEff.id] and eff.turnLeft > 0:
					...
					self.hp = 1
					ballerine = add_effect(caster=self, target=self, effect=undeadEff2, start=eff.effects.name, skillIcon=eff.icon)
					...
					if self.status == STATUS_ALIVE:
						...
						add_effect(eff.caster,self, effect("Résurection récente","susRez",overhealth=shieldValue,turnInit=3,absolutShield=True,emoji=sameSpeciesEmoji('<:naB:908490062826725407>','<:naR:908490081545879673>')),ignoreEndurance=True)
						self.refreshEffects()
		...
		for indx, tmpId in enumerate(self.char.equippedChips):
			...
			if tmpChip != None:
				if tmpId == getChip("Totem de Protection").id and self.chipAddInfo[indx]:
					...
					eff1, eff2 = effect("Protection","immortalityTotemShield",PURCENTAGE,type=TYPE_ARMOR,trigger=TRIGGER_DAMAGE,overhealth=tmpUsrChip.power,absolutShield=True), effect("Régénération","immortalityTotemHeal",PURCENTAGE,type=TYPE_INDIRECT_HEAL,trigger=TRIGGER_START_OF_TURN,power=tmpUsrChip.power,turnInit=3,emoji=tmpChip.emoji)
		...
		for tmpEnt in self.otherVars["benediction"][self.team]:
			tmpEnt = self.entDict[tmpEnt]
			for indx, tmpId in enumerate(tmpEnt.char.equippedChips):
				...
				if tmpChip != None:
					if tmpId == getChip("Bénédiction").id and tmpEnt.chipAddInfo[indx]:
						...
						self.otherVars["benediction"][self.team].remove(tmpEnt.id)
						return tmpMsg
		...
		if self.isNpc("Aformité incarnée") and dictIsNpcVar["Luna prê."]:
			if not(killer.isNpc("Luna prê.")):
				...
				add_effect(self,killer,effect("Borgne","lunaBlinded",precision=-50,stat=MAGIE,turnInit=-1,unclearable=True))
		...
		try: # death reaction
			...
			for reactNames, reactValues in self.char.says.specDeath.items():
				if dictIsNpcVar[reactNames]:
					for entId, ent in self.entDict.items():
		...
		for fEffect in self.effects:
			if fEffect.effects.trigger == TRIGGER_DEATH:
				...
				if self.entDict[fEffect.caster.id].hp > 0:
					temp1, temp2 = self.entDict[fEffect.caster.id].heal(target=entDict[fEffect.caster.id], icon=fEffect.icon, statUse=FIXE, power=int(fEffect.value*DEEP_WOUND_RATIO/100), lifeSteal = True, direct=False, incHealJauge=False)
					pipistrelle += temp1
				...
				stat,damageBase,allReadySeen, killCount, dmgDict, beforeDmg, tempMsg, nbHit = fEffect.caster.allStats[MAGIE]-fEffect.caster.actIndirect,summationPower,[], 0 , "", copy.deepcopy(fEffect.caster.stats.indirectDamageDeal), "", 0
				for a in self.cell.getEntityOnArea(area=secretum.effects.area,team=self.team,wanted=ALLIES,directTarget=False,fromCell=actTurn.cell):
					...
					damage, tempLogs = indirectDmgCalculator(fEffect.caster,self,summationPower,MAGIE,[100, self.danger][fEffect.caster.team], AREA_CIRCLE_2)
					damage = damage*reduc
		...
		if self.char.npcTeam == NPC_BOMB:
			tmpEff = effect("Explosion",self.skills[0].id,self.skills[0].use,area=self.skills[0].area,type=TYPE_DAMAGE,trigger=TRIGGER_INSTANT,power=self.skills[0].power,emoji=self.char.icon[self.team])
		...
		if dispersionDict != {}:
			...
			for ent, tPower in dispersionDict.items():
				...
				tmpEff = effect("Dispersion ({0})".format(tPower),"dispEff",HARMONIE,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_END_OF_TURN, power=tPower, emoji=tmpChip.emoji, replace=True, turnInit=3)
		...
		if killer.char.aspiration == SORCELER and type(self.char) not in [invoc, depl] and killer != self:
			for a in self.cell.getEntityOnArea(area=AREA_CIRCLE_1,team=self.team,wanted=ALLIES,directTarget=False,fromCell=actTurn.cell):
				...
				if a != self:
					reduc = max(AOEMINDAMAGE,1-self.cell.distance(a.cell)*AOEDAMAGEREDUCTION)
				damage, tempLogs = indirectDmgCalculator(killer, a, max(5,killer.char.level), MAGIE, [100, self.danger][killer.team], AREA_CIRCLE_1)
				damage = damage * reduc 
		...
		if self.status == STATUS_DEAD:
			...
			pipistrelle += killer.heal(killer,"",CHARISMA,60, effName = "Âme consummée", mono = True, useActionStats = ACT_HEAL,direct=False)[0]

		if self.char.__class__ == octarien and self.isNpc("Tour"):
			for ent in self.tablEntTeam[self.team]:
		...
		if type(self.char) not in [invoc, depl] and killer != self or killer.team != self.team: killer.stats.ennemiKill += 1
		elif type(self.char) not in [invoc, depl] and killer != self and killer.team == self.team: killer.stats.friendlyfire += 1
		return pipistrelle

	def attack(self, target: "entity"= None, value = 0, icon = "", area=AREA_MONO, accuracy=None, use=STRENGTH, onArmor = 1, effectOnHit = [], useActionStats = ACT_DIRECT, setAoEDamage = False, lifeSteal:int = 0, skillUsed: Union[skill, weapon] = None, actTurn=None) -> str:
		if effectOnHit == None: effectOnHit = []
		elif effectOnHit.__class__ != list: effectOnHit = [effectOnHit]
		if accuracy==None: accuracy=self.char.weapon.accuracy
		if useActionStats == None: useActionStats = ACT_DIRECT
		if skillUsed == None: skillUsed = skill("No Skill","",TYPE_UNIQUE)
		sumDamage, deathCount = 0, 0
		popipo, critMsg = "", ""
		skillPercing = skillUsed.percing
		erosion = skillUsed.erosion

		if target != None and value > 0: # If the target is valid
			dangerMul, dodgeMul, armorSteal, armorStealEmojis, effDealsDamage, backupDancerFlag, backupbisFlag = 1, 1, 0, "", [], None, None
			if self.team: dangerMul = self.danger/100

			if type(skillUsed) == skill and skillUsed.armorSteal > 0:
			...
			if type(skillUsed) == skill and skillUsed.armorConvert > 0:
			...
			if self.hp > 0: # Does the attacker still alive ?
				critMalusCmpt, tablTarget, lifeStealMsg, aoeStealMsg, armorStolen, lifeStealEmote, tablPanhaima = 0, target.cell.getEntityOnArea(area=area, team=self.team, wanted=ENEMIES, directTarget=False, fromCell=self.cell), "","", 0, [], []
				for entityInArea in tablTarget: # For eatch ennemis in the area
					if entityInArea.hp > 0: # If the entityInArea is alive
						...
						if entityInArea.isNpc(TGESL1.name):
							entityInArea = self.entDict[entityInArea.summoner.id]
						self.logs += "===== {0} is attacking {1} =====\n".format(self.name,entityInArea.name)
						critMsg = ""
						...
						self.logs += "- Hit Rate : {0}% -> ".format(succRate/10)
						if random.randint(0,999) < succRate:
							...
							if skillUsed.id != requiem.id:
								...
								else: aoeFactor = 1-(min(1-AOEMINDAMAGE,min(entityInArea.cell.distance(target.cell)-int(target==self or area in [AREA_DONUT_1,AREA_DONUT_2,AREA_DONUT_3]),0)*AOEDAMAGEREDUCTION)); self.logs += "\n- AoEDamage : {0}%".format(aoeFactor*100)
							else:
							...
							if entityInArea.immune: damage = 0
								...
								damage, dmgBoostDict, resisted = dmgCalculator(self, entityInArea, value, use, useActionStats, area, typeDmg = TYPE_DAMAGE, skillPercing = skillPercing)
								self.logs += "\n- Damage : " + str(damage)
								...
								for entBooster, value2 in dmgBoostDict.items():
									if entBooster.id != self.id:
										if value2 > 0:
											if entBooster.char.__class__ not in [invoc, depl]: self.entDict[entBooster.id].stats.damageBoosted += round(value2*aoeFactor)
											else: self.entDict[entBooster.summoner.id].stats.damageBoosted += round(value2*aoeFactor)
											self.stats.underBoost += round(value2*aoeFactor)
										elif value2 < 0:
											if entBooster.char.__class__ not in [invoc, depl]: self.entDict[entBooster.id].stats.damageDodged += round(abs(value2*aoeFactor))
											else: self.entDict[entBooster.summoner.id].stats.damageDodged += round(abs(value2*aoeFactor))
								...
								for tmpEff in entityInArea.effects:
									incDmg = 0
									if tmpEff.effects.id in [vulne.id,defenseUp.id,iliStans1_1.id] or tmpEff.effects.id in [krysWaterWeakness.id] and (type(skillUsed) == skill and skillUsed.condition==[EXCLUSIVE,ELEMENT,ELEMENT_WATER]): incDmg = damage * tmpEff.effects.power/100 * [-1, 1][tmpEff.effects.type in hostileTypes]
									...
									elif tmpEff.effects.id == elemResistanceEffects[1].id and (type(skillUsed) == skill and len(skillUsed.condition)>0 and skillUsed.condition[:2]==[EXCLUSIVE,ELEMENT] and skillUsed.condition[2] in range(ELEMENT_FIRE,ELEMENT_EARTH+1)): incDmg = damage * ELEMENT_RESIST/100 * [-1,1][skillUsed.condition[2] == elemWeakness[entityInArea.char.element]] * ((100-ELEMENT_RESIST)/100)
							...
							if ((type(skillUsed) == skill and skillUsed.group == SKILL_GROUP_HOLY and entityInArea.char.npcTeam in [NPC_UNDEAD,NPC_DEMON])) or (type(skillUsed) == skill and skillUsed.group == SKILL_GROUP_DEMON and entityInArea.char.npcTeam in [NPC_HOLY,NPC_ANGEL]): dmgMul = dmgMul*((100+UNDEAD_WEAKNESS)/100)
							...
							if ((type(skillUsed) == skill and skillUsed.garCrit and entityInArea.id == target.id) or (self.weaponEffect([eclataDash.id, eclataDash.id+"+",victoireShotgunEff.id]) and self.cell.distance(entityInArea.cell)<=1)) and entityInArea.id == target.id: critRate = 100
							...
							if critRoll < critRate and damage > 0:
								...
								self.logs += " - Crit. Mul : {0}%".format(critDmgMul*100)
								critMsg = " !"

								if (skillUsed != None and skillUsed.garCrit and entityInArea.id == target.id): critMsg += "!"

							else: self.logs += "Failure"; critDmgMul += self.dmgUp / 100

							# Block
							self.logs += "\n- entityInArea Block Rate : {0}% -> ".format(entityInArea.blockRate)
							if type(skillUsed) == skill and skillUsed.id == trans.id and entityInArea.isNpc("Iliana OvL","Nacialisla"): blocked = True
							if random.randint(0,99) > 100-entityInArea.blockRate or blocked: self.logs += "Success"; critMsg += " (Bloqué !)"; blocked = True
							else: self.logs += "Failure"
							...
							if entityInArea.hp/entityInArea.maxHp <= 0.35 and entityInArea.char.__class__ not in [invoc, depl] and self.otherVars["sacrificialShield"][entityInArea.team] != [] and effectiveDmg > 0:
								for ent in self.otherVars["sacrificialShield"][entityInArea.team]:
							...
							for indx, chipId in enumerate(self.char.equippedChips):
								...
								if tmpChip != None:
									...
									if tmpChip.name == "Diffraction" and area==AREA_MONO and len(self.getEntityInMelee(entityInArea))>=1 and type(skillUsed) == skill and not(skillUsed.replay) and effectiveDmg > 35:
										difDmg = int(effectiveDmg * self.char.chipInventory[tmpChip.id].power/100)
										effectiveDmg -= difDmg
										difEff = effect(tmpChip.name,"difEff",None,type=TYPE_INDIRECT_DAMAGE,area=AREA_DONUT_2,trigger=TRIGGER_INSTANT,power=difDmg,emoji=tmpChip.emoji)
							...
							if huntMark: 
								...
								try:
									if self.otherVars["teamJaugeDict"][self.team][self].effects.id == lycanJauge.id: self.otherVars["teamJaugeDict"][self.team][self].value = min(self.otherVars["teamJaugeDict"][self.team][self].value+5,100)
								except KeyError: pass
							self.stats.shootHited += 1
							if entityInArea != self:
								if type(self.char) not in [invoc,depl]: self.stats.damageDeal += effectiveDmg
								else: self.entDict[self.summoner.id].stats.damageDeal += effectiveDmg; self.entDict[self.summoner.id].stats.summonDmg += effectiveDmg
							...
							try:
								if not(skillUsed.__class__ == skill and skillUsed.jaugeEff != None and skillUsed.jaugeEff.id not in [bloodJauge.id, clemTmpBloodJauge.id]):
									for conds in self.otherVars["teamJaugeDict"][self.team][self].effects.jaugeValue.conds:
										if conds.type == INC_DEAL_DAMAGE: self.otherVars["teamJaugeDict"][self.team][self].value = min(self.otherVars["teamJaugeDict"][self.team][self].value+((conds.value*effectiveDmg/entityInArea.maxHp*100)*([1,5][entityInArea.char.standAlone])),100)
										elif conds.type == INC_ON_HIT: self.otherVars["teamJaugeDict"][self.team][self].value = min(round(self.otherVars["teamJaugeDict"][self.team][self].value+(conds.value*[0.3,1][entityInArea.id == target.id or (skillUsed == skill and skillUsed.replay) or (skillUsed != None and skillUsed.repetition > 1)]),2),100)
							except KeyError: pass
							...
							if entityInArea.hp <= 0:
								lolipop = entityInArea.death(killer = self)
								if (entityInArea.char.__class__ not in [invoc, depl]) or (entityInArea.char.__class__ in [invoc, depl] and len(lolipop.splitlines()) > 1): popipo += lolipop
							...
							if hasCrit:
								for jaugeEnt, jaugeEff in self.otherVars["teamJaugeDict"][self.team].items():
									for conds in jaugeEff.effects.jaugeValue.conds:
										if (conds.type == INC_ON_SELF_CRIT and jaugeEnt == self) or (conds.type == INC_ON_ALLY_CRIT and jaugeEnt.hp > 0) : 
											jaugeEff.value = min(round(jaugeEff.value+(conds.value*[0.3,1][entityInArea.id == target.id and (skillUsed != None and (type(skillUsed) == skill and not(skillUsed.replay) or skillUsed.repetition <= 1))]),2),100)
							...
							if (entityInArea.isNpc("Liu","Kitsune") and entityInArea.team == 1) and type(actTurn.char != invoc):
							...
							for eff in entityInArea.effects:
								...
								elif eff.effects.id == anticipationEff2 and (type(skillUsed) == skill and len(skillUsed.condition) > 1 and skillUsed.condition[1] == ELEMENT):
									popipo += add_effect(eff.caster,entityInArea,eff.effects.callOnTrigger)
									try:
										self.otherVars["teamJaugeDict"][entityInArea.team][entityInArea].value = min(self.otherVars["teamJaugeDict"][entityInArea.team][entityInArea].value+eff.effects.power,100)
									except KeyError:
										pass
								...
								elif eff.effects.id == deathMark.id and type(skillUsed) == skill and skillUsed.jaugeEff != None and skillUsed.jaugeEff.id == planJauge.id and skillUsed.minJaugeValue > 1:
									tmpEff = effect("Marque de la Mort","deathMarkTriggered",MISSING_HP,power=round(max(10,50*skillUsed.minJaugeValue/100)),type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_INSTANT,emoji=deathMark.emoji)
							...
							if backupDancerFlag:
								...
								if totalShieldValue == 0:
									tmpValue = 1
									if type(skillUsed) in [skill,weapon] and skillUsed.onArmor != 1: tmpValue = skillUsed.onArmor
									tmpEff = effect("Rupture Extrême", backupDancerFlag.effects.id+"1", HARMONIE, type=TYPE_INDIRECT_DAMAGE, trigger=TRIGGER_INSTANT, emoji=backupDancerFlag.icon, power=15+min(max(round((self.getOnArmorDmgValue(tmpValue)-1)*backupDancerFlag.effects.power),0),round(85*backupDancerFlag.effects.power/100)))
							...
							try: 
								if self.otherVars["teamJaugeDict"][self.team][self].effects.id == lycanJauge.id and target.id == entityInArea.id and not (huntMark) and target.id != self.id:
									...
									popipo += groupAddEffect(self, entityInArea, AREA_MONO, hunterMarkEff, self.otherVars["teamJaugeDict"][self.team][self].icon)
							except KeyError: pass

							try: 
								if self.otherVars["teamJaugeDict"][self.team][self].effects.id == bloodJauge.id and random.randint(0,99) <= [30,50][entityInArea.id == target.id] and target.id != self.id:
									popipo += groupAddEffect(self, entityInArea, AREA_MONO, bloodButterfly, self.otherVars["teamJaugeDict"][self.team][self].icon)
							except KeyError: pass

							try: 
								if self.otherVars["teamJaugeDict"][self.team][self].effects.id == planJauge.id and target.id == entityInArea.id and target.id != self.id and not(skillUsed.jaugeEff != None and skillUsed.jaugeEff.id == planJauge.id and skillUsed.minJaugeValue > 1):
									isEligible = True
									for eff in entityInArea.effects:
										if eff.effects.id == deathMark.id: isEligible = False; eff.leftTurn = deathMark.turnInit; break
									
									if isEligible: popipo += groupAddEffect(self, entityInArea, AREA_MONO, deathMark, self.otherVars["teamJaugeDict"][self.team][self].icon)
							except KeyError: pass
						else:
							self.logs += "Miss"
						...
						self.logs += "\n\n"
				if len(tablTarget) > 0 and (self.isNpc("Liz","Kitsune")) and self.team == 1 and ((type(skillUsed) == skill and skillUsed.id != kitsuneSkill4.id) or skillUsed == None):
					popipo += ["","\n"][popipo[-2:-1] != "\n"] + groupAddEffect(caster=self, target=target,area=tablTarget, effect=charming, skillIcon=self.weapon.effects.emoji[0][0])

				if armorStolen > 0 and sumDamage <= 0 and type(self.char) not in [depl]:
					eff = stolenArmorEff
				...
				elif sumDamage > 0 and type(self.char) not in [depl]:
					...
					if armorConvert > 0 or armorStolen > 0:
						...
						eff.overhealth = toAdd = calShieldValue(self,self,sumDamage*((armorConvert+lythoBonus)/100),FIXE,self.turnNumber)[0]+armorStolen
					...
					if aoeArmorConvertBoost > 0:
						tablTarget, supposedArmor = self.cell.getEntityOnArea(area=AREA_DONUT_2,team=self.team,wanted=ALLIES,directTarget=False,fromCell=actTurn.cell), calShieldValue(self,self,sumDamage * (skillUsed.aoeArmorConvert / 100),FIXE,self.turnNumber)[0]
						eff = copy.deepcopy(convertArmor)
					...
					if skillUsed != None and skillUsed.id == clemSkill7.id:
						...
						if nbHeal > 0:
							...
							effList.append(eff)
						if nbBoost > 0:
							eff = effect("Prise de Sang","clemBoost", stat=PURCENTAGE,magie=2.5*nbBoost, turnInit=skillUsed.cooldown, emoji='<:pacteDeSang:917096147452035102>')
							effList.append(eff)
						if len(effList) > 0:
	...
	def startOfTurn(self) -> Union[str, timeline]: 
		"""
		...
		for eff in self.effects:
			if eff.trigger == TRIGGER_START_OF_TURN:
				hpTargetBefore, hpCasterBefore = self.hp, eff.caster.hp
				ballerine = eff.triggerStartOfTurn()
				tmpMsg += ballerine

				if eff.effects.type == TYPE_INDIRECT_DAMAGE and eff.effects.area == AREA_MONO:
					listIndEff.append([eff.caster.icon,eff.icon,hpTargetBefore-self.hp])
					if eff.caster.hp > hpCasterBefore: listIndLifeSteal.append([eff.caster.icon,eff.icon,eff.caster.hp-hpCasterBefore])
				elif eff.effects.type == TYPE_INDIRECT_HEAL and eff.effects.area == AREA_MONO: listIndHealEff.append([eff.caster.icon,eff.icon,self.hp-hpTargetBefore])
				else: toReturn += ballerine

			elif eff.effects.id == gwenCoupeEff.id:
				if random.randint(0,99) < 15:
					eff = intargetable
					toReturn += add_effect(self,self,effect)
			elif eff.effects.id == smallBombEff.id:
				self.skills[0].power += self.skills[0].initPower
			if eff.effects.replica != None:
				isCasting = True

			if eff.effects.decateOnTurn:
				temp = eff.decate(turn=1)
				if eff.effects.trigger in [TRIGGER_ON_REMOVE,TRIGGER_ON_MOVE]:
					toReturn += temp
		...
		for indx, chipId in enumerate(self.char.equippedChips):
			...
			if tmpChip != None:
				if tmpChip.name in toResetChip:
				...
				elif tmpChip.name in ["Médaille de la Chauve-Souris","Haut-Perceur 5.1","Cadeau Explosif"] and (self.turnNumber-1)%3==0 and self.hp > 0:
					...
					toSmn.skills[-1] = skill(tmpChip.name,tmpChip.name,TYPE_PASSIVE,effects=[batHpBoost,batDmgBoost],emoji=tmpChip.emoji)

					cellToSummon = self.cell.getCellForSummon(AREA_DONUT_2,self.team,toSmn,self)
					if cellToSummon != None:
						tempBlabla = self.summon(toSmn, cellToSummon)
						toReturn += tempBlabla
						self.logs += "\n"+tempBlabla
					elif toSmn.npcTeam == NPC_BOMB:
						tmpEff = effect("Explosion",toSmn.skills[0].id,toSmn.skills[0].use,area=toSmn.skills[0].area,type=TYPE_DAMAGE,trigger=TRIGGER_INSTANT,power=toSmn.skills[0].power,emoji=toSmn.icon[self.team])
				...
				elif tmpChip.name == "Pas Piégés" and (self.turnNumber-1)%3==0:
		...
		try:
			for conds in self.otherVars["teamJaugeDict"][self.team][self].effects.jaugeValue.conds:
				if conds.type == INC_START_TURN: self.otherVars["teamJaugeDict"][self.team][self].value = min(self.otherVars["teamJaugeDict"][self.team][self].value+conds.value,100); break
		...
		if self.hp > 0:
			if TRAIT_ARACHNOPHOBE in self.char.trait:
				for celly in self.cell.getSurrondings():
					if celly != None and celly.on != None and celly.on.char.npcTeam == NPC_SPIDER:
						arachnoEff = effect("Arachnophobie","arachnoEff", stat=PURCENTAGE,strength=-10,magie=-10,charisma=-10,intelligence=-10,emoji='<:osaSmn21:1171873951673237537>',silentRemove=True)
						temp = groupAddEffect(caster=celly.on, target=self, area=AREA_MONO, effect=arachnoEff)
						if celly.on.team != self.team:
							if celly.on.char.__class__ == invoc and celly.on.name.startswith("Myrlopé") and self.isNpc("Hélène"):
								if len(temp) > 0:
								...
								temp += temp2[0]
								if celly.on.summoner != None and celly.on.summoner.isNpc("Shehisa") and self.entDict[celly.on.summoner.id].hp > 0:
									temp2 = "{0} : *\"{1}\"*\n".format(celly.on.summoner.icon,randRep(shehisaReactAttackMyrlope)) + self.entDict[celly.on.summoner.id].counter(target=self)
									if len(temp)>0:
										temp += ["\n",""][temp[-1] == "\n" or temp2[0] == "\n"]+temp2
									else:
										temp += temp2
			...
			if TRAIT_ARACHNOPHILE in self.char.trait:
				for celly in self.cell.getSurrondings():
					if celly != None and celly.on != None and celly.on.char.npcTeam == NPC_SPIDER:
						arachnoEff = effect("Arachnophile","arachnoEff", stat=PURCENTAGE,strength=10,magie=10,charisma=10,intelligence=10,emoji='<:osaSmn21:1171873951673237537>',silentRemove=True)
			...
			if self.char.npcTeam == NPC_DRYADE:
				tmpEff = effect("Photosynthèse",constEff.id,PURCENTAGE,power=self.turnNumber/2,strength=self.turnNumber/2,endurance=self.turnNumber/2,charisma=self.turnNumber/2,agility=self.turnNumber/2,precision=self.turnNumber/2,intelligence=self.turnNumber/2,magie=self.turnNumber/2,emoji='<:lifeSeed:1028680000628596817>',silent=True)
		...
		if self.char.aspiration == MASCOTTE:
			...
			if nbEnnemiHit > 0:
				self.aspiSlot = []
				enchMagBuff = effect("Enchanteur","enchMagBuff",silent=True,emoji=aspiEmoji[ENCHANTEUR],magie=self.baseStats[MAGIE]*min(0.05*nbEnnemiHit,0.3))
				add_effect(self, self, enchMagBuff)

		if self.team == 0:
			nbAliveAllies, lenTeam = 0, 0
			for ent in self.tablEntTeam[self.team]:
				if type(ent.char) in [tmpAllie, char]:
					if ent.hp > 0: nbAliveAllies += 1
					lenTeam += 1
			if self.turnNumber >= LAST_DITCH_EFFORT_START_TURN or nbAliveAllies/lenTeam <= LAST_DITCH_EFFORT_START_ALLIE:
				turnPowa, alliesPowa = ((self.turnNumber-LAST_DITCH_EFFORT_START_TURN)/(LAST_DITCH_EFFORT_MAX_TURN-LAST_DITCH_EFFORT_START_TURN))*LAST_DITCH_EFFORT_MAX_POWER*int(self.turnNumber >= LAST_DITCH_EFFORT_START_TURN), (((nbAliveAllies/lenTeam)+LAST_DITCH_EFFORT_MAX_ALLIE)/(LAST_DITCH_EFFORT_START_ALLIE+LAST_DITCH_EFFORT_MAX_ALLIE))*LAST_DITCH_EFFORT_MAX_POWER*int(nbAliveAllies/lenTeam <= LAST_DITCH_EFFORT_START_ALLIE)
	...
	def endOfTurn(self) -> str:
		...
		for eff in self.effects:
			if eff.effects.id == deepWound.id:
				tmpEff = effect(deepWound.name,deepWound.id+"1",power=int(eff.value*DEEP_WOUND_RATIO/100),trigger=TRIGGER_END_OF_TURN,type=TYPE_INDIRECT_DAMAGE,emoji=deepWound.emoji,silentRemove=True,stackable=True)
		...
		for eff in self.effects:
			if eff.trigger == TRIGGER_END_OF_TURN:
				hpTargetBefore, hpCasterBefore = self.hp, eff.caster.hp
				ballerine = eff.triggerEndOfTurn()
				tmpMsg += ballerine
		...
		if type(self.char) == invoc:
			...
			if self.leftTurnAlive <= 0 and self.hp > 0:
				...
				entDict[self.summoner.id].ownEffect = self.entDict[self.summoner.id].ownEffect+self.ownEffect

		for indx, chipId in enumerate(self.char.equippedChips):
			tmpChip = getChip(chipId)
			if tmpChip != None:
				if tmpChip.name == "Radiance Toxique" and self.chipAddInfo[indx] > 1:
					tmpEff = effect(tmpChip.name,"radTox",FIXE,type=TYPE_INDIRECT_HEAL,lvl=int(self.chipAddInfo[indx]),trigger=TRIGGER_INSTANT,area=AREA_CIRCLE_2,emoji=tmpChip.emoji,lifeSteal=True)
					...
					for tmpEnt in self.tablEntTeam[not(self.team)]:
						if tmpEnt.hp > 0 and type(tmpEnt.char) not in [invoc, depl]:
					...
					tmpEff1 = effect(tmpChip.name,tmpChip.name,PURCENTAGE,strength=chipPow,endurance=chipPow,charisma=chipPow,agility=chipPow,precision=chipPow,intelligence=chipPow,magie=chipPow,silentRemove=True)
					...
					tmpEff = effect(tmpChip.name,tmpChip.name,type=TYPE_INDIRECT_DAMAGE,power=int(self.chipAddInfo[indx]*self.char.chipInventory[chipId].power/100),trigger=TRIGGER_INSTANT,emoji=tmpChip.emoji)
					...
					tmpEff = effect(tmpChip.name,tmpChip.name,FIXE,type=TYPE_INDIRECT_HEAL,lvl=int(self.chipAddInfo[indx]*self.char.chipInventory[chipId].power/100),trigger=TRIGGER_INSTANT,emoji=tmpChip.emoji)
	...
	async def resurect(self, target, value : int, icon : str):
		if self.turnNumber <= 20 and target.status != STATUS_TRUE_DEATH:
			...
			# Recent Rez shield
			notDeadYet = effect("Résurection récente","susRez",overhealth=shieldValue,turnInit=3,absolutShield=True,emoji=sameSpeciesEmoji('<:naB:908490062826725407>','<:naR:908490081545879673>'))
			add_effect(self,target,notDeadYet,ignoreEndurance=True)
			if target.isNpc("Akira"):
				mater = effect("Rage Inhumaine","Akiki",strength=int(target.char.level*1.5),turnInit=-1,emoji="<a:menacing:917007335220711434>",stackable=True)
	...
	def heal(self,target, icon : str, statUse : Union[int,None], power : int, effName = None, mono = False, useActionStats = ACT_HEAL, lifeSteal = False, direct=True, incHealJauge=True):
		...
		if power > 0 and target.hp > 0:
			...
			if statUse not in [None,FIXE,PURCENTAGE,MISSING_HP]:
				...
				healPowa, elemMul = calHealPower(self, target, power, 1, statBucket, useActionStats), 1
				...
				if bloodButterflyFlag != None and healPowa > 0:
					...
					tmpEff = effect("Vol de Soins","bloodButterflyLifeSteal",FIXE,type=TYPE_INDIRECT_HEAL,lvl=reducedHeals,turnInit=20,trigger=TRIGGER_START_OF_TURN,emoji='<:bloodButterflyHealing:1257770648277618770>',lifeSteal=True)
				...
				if type(self.char) != octarien:
					if not(target.auto): target.healResist += int(healPowa/target.maxHp/2.5*100)
					else: target.healResist += int(healPowa/target.maxHp/3.5*100)
				else:
			...
			if self.char.__class__ in [invoc, depl]: toLook1, toLook2, chipMul = [entDict[self.summoner.id].char.equippedChips], [entDict[self.summoner.id].chipAddInfo], 0.4
						...
						try:
							for conds in self.otherVars["teamJaugeDict"][self.team][[self,self.summoner][type(self.char) in [invoc,depl]]].effects.jaugeValue.conds:
								if conds.type == INC_DEAL_HEALING or (conds.type == INC_LIFE_STEAL and lifeSteal):
									self.otherVars["teamJaugeDict"][self.team][[self,self.summoner][type(self.char) in [invoc,depl]]].value = min(self.otherVars["teamJaugeDict"][self.team][[self,self.summoner][type(self.char) in [invoc,depl]]].value+(conds.value*(healPowa/target.maxHp*100))*(1-(0.5*int(direct))),100)
						except KeyError:
							pass
			...
			if  self.weaponEffect([lioWeap, kitsuneWeap]) and self.team == 1 and type(target.char != invoc):
	...
	def getCellToMove(self,cellToMove:cell=None,lastCell:cell=None):
		if cellToMove == None:
			tablOfEnt = self.cell.getEntityOnArea(area=AREA_ALL_ENEMIES-int(self.char.weapon.target==ALLIES), team=self.team, wanted=self.char.weapon.target, lineOfSight=self.char.weapon.target==ENEMIES, ignoreInvoc = True)
	...
	def knockback(self, target, power:int, fromEnt=None):
		...
		posArea = []
		for cellule in self.tablAllCells:
			...
			try:
				for conds in self.otherVars["teamJaugeDict"][self.team][self].effects.jaugeValue.conds:
					if conds.type == INC_PER_PUSH:
						self.otherVars["teamJaugeDict"][self.team][self].value = min(self.otherVars["teamJaugeDict"][self.team][self].value+(conds.value*cellToPush.distance(initTagetCell)),100)
			except KeyError:
				pass
		...
		if cmpt < power:
			...
			for boom in badaboum:
				...
				damage, tempLogs = indirectDmgCalculator(self,boom,dmgBase,HARMONIE,[100, self.danger][self.team],AREA_MONO)
				try:
				...
				try:
					for conds in self.otherVars["teamJaugeDict"][self.team][self].effects.jaugeValue.conds:
						if conds.type == INC_COLISION:
							self.otherVars["teamJaugeDict"][self.team][self].value = min(self.otherVars["teamJaugeDict"][self.team][self].value+conds.value,100)
				except KeyError:
					pass
	...
	def smnDepl(self, depl, cell):
		if cell.depl == None:
			...
			deplEnt = entity(identifiant=self.id, character = depl, team=self.team, player=False, auto=True, summoner=self, cell=cell)
			cell.depl = deplEnt
			self.deplTabl.append([deplEnt,cell,cell.getArea(area=[depl.skills.area,depl.triggerArea][depl.trap],team=self.team,fromCell=cell)])
			if depl.lifeTime > 0:
	...
	def counter(self,target,counterOnDodge=0):
		...
		if canCounter:
			actStat, tablActStats = ACT_INDIRECT, [self.actHeal,self.actBoost,self.actShield,self.actDirect,self.actIndirect]
			...
			damage, tempLogs = indirectDmgCalculator(self, target, COUNTERPOWER*(1+counterDmgBuff), HARMONIE, area=AREA_MONO, useActionStat=actStat)
	...
	async def changeNpc(self,char: Union[char,octarien,tmpAllie,invoc]):
		for eff in self.effects:
	...
	def summonMemoria(self, target, timeline):
		if type(self.char, ) == invoc: return ""
			...
			if target.char.team != NPC_UNDEAD:
				...
				if cellToSummon != None:
					funnyTempVarName = actTurn.summon(smn, cellToSummon)
				...
				try:
					for conds in self.otherVars["teamJaugeDict"][self.team][self].effects.jaugeValue.conds:
						if conds.type == INC_JUMP_PER_CASE:
							self.otherVars["teamJaugeDict"][self.team][self].value = min(self.otherVars["teamJaugeDict"][self.team][self].value+(conds.value*initCell.distance(self.cell)),100)
				except KeyError:
					pass
	...
	def passiveInitialisation(self):
		if self.isNpc("Clémence"):
			...
			if not(isBossAgaints):
				...
				self.icon = self.char.splashIcon
				youngClemEff1 = effect("Jeunesse maudite","youngClemEff",PURCENTAGE,type=TYPE_MALUS,strength=-10,endurance=-15,magie=-10,turnInit=-1,silent=True,unclearable=True,stackable=True,emoji='<:isa:1158136337061400797>')
				youngClemEff2 = effect("Jeunesse maudite","youngClemEff",PURCENTAGE,charisma=15,agility=15,dodge=10,turnInit=-1,silent=True,unclearable=True,stackable=True,emoji='<:isa:1158136337061400797>')
		...
		if self.char.npcTeam == NPC_ANGEL:
			for indx, tmpSkill in enumerate(self.skills):
				if tmpSkill.__class__ == skill:
					...
					if tmpSkill.effectAroundCaster not in [None,[]] and type(tmpSkill.effectAroundCaster[2]) == effect and tmpSkill.effectAroundCaster[2].stat in range(MAGIE+1):
						tmpEff = copy.deepcopy(tmpSkill.effectAroundCaster[2])
						tmpEff.stat = HARMONIE
						tmpSkill.effectAroundCaster[2] = tmpEff
					
					if tmpSkill.effectOnSelf not in [None] and type(tmpSkill.effectOnSelf) == effect and tmpSkill.effectOnSelf.stat in range(MAGIE+1):
		...
		try:
			if self.char.elemAffinity:
				temp = add_effect(self,self,elemResistanceEffects[self.char.element])
		...
		for indx, charSkill in enumerate(self.skills):
			if type(charSkill) == skill:
				if charSkill.jaugeEff != None:			   # If the entity have a passive
					temp = add_effect(self,self,charSkill.jaugeEff," ({0})".format(charSkill.name))
					if '🚫' not in temp:
		...
		for equip in [self.char.weapon]+self.char.stuff:	   # Look at the entity stuff to see if their is a passive effect
			if equip.effects != None:
				...
				if eff != None and not(self.char.__class__ == invoc and eff.type == TYPE_SUMMON):
					temp = add_effect(self,self,eff," ({0})".format(equip.name))
					if '🚫' not in temp:
						listEffGiven.append(eff.emoji[self.char.species-1][self.team])
						listEffGiver.append(equip.emoji)
						self.logs += "{0} get the {1} eff from {2}\n".format(self.char.name,eff.name,equip.name)
		...
		if self.char.element == ELEMENT_LIGHT:
			...
			temp = add_effect(self,self,temp)
					...
					for cmpt, chipName in enumerate(toSetChipName):
						if tmpChip.name == chipName:
							...
							if chipName == "Bénédiction":
								self.otherVars["benediction"][self.team].append(self.id)
				if tmpChip.name == "Bouclier Sacrificiel":
					self.otherVars["sacrificialShield"][self.team].append(self)
				elif tmpChip.name == "Armure Initiale":
					initArmor = effect(tmpChip.name,"initArmor",overhealth=self.maxHp*self.char.chipInventory[chipId].power/100,turnInit=5,type=TYPE_ARMOR,trigger=TRIGGER_DAMAGE,replace=True,emoji=tmpChip.emoji)
	...
	def triggerDepl(self):
		...
		if self.skills.type in [TYPE_BOOST,TYPE_ARMOR,TYPE_INDIRECT_HEAL,TYPE_INDIRECT_REZ,TYPE_INDIRECT_DAMAGE,TYPE_MALUS] and self.skills.effects != [None]:
			...
			for ent in self.cell.getEntityOnArea(area=self.skills.area,team=self.team,fromCell=self.cell,wanted=ALLIES,directTarget=False):
				funnyTempVarName, healValue = self.heal(ent, self.skills.emoji, self.skills.use, self.skills.power, mono = self.skills.area == AREA_MONO, useActionStats = self.skills.useActionStats)
				self.logs += funnyTempVarName
				if healValue > 0:
		...
		if self.skills.effectAroundCaster != None:
			...
			if self.skills.effectAroundCaster[0] == TYPE_HEAL:
				...
				for ent in self.cell.getEntityOnArea(area=self.skills.effectAroundCaster[1],team=self.team,fromCell=self.cell,wanted=ALLIES,directTarget=False):
					funnyTempVarName, healValue = self.heal(ent, self.skills.emoji, self.skills.use, self.skills.effectAroundCaster[2], mono = self.skills.effectAroundCaster[1] == AREA_MONO, useActionStats = self.skills.useActionStats)
					self.logs += funnyTempVarName
					if healValue > 0:
						entHealedDict[ent] = healValue
				...
				toReturn += funnyTempVarName
				self.logs += "\n"+funnyTempVarName
			elif type(self.skills.effectAroundCaster[2]) == effect or findEffect(self.skills.effectAroundCaster[2]) != None :
	...
	def weaponEffect(self, weapEffId: Union[str, effect, weapon, List[Union[str, effect, weapon]]]):
		if type(weapEffId) != list:
			weapEffId = list(weapEffId)
		
		for obj in weapEffId:
			if type(obj) == effect: obj = obj.id
			elif type(obj) == weapon:
...
class fightEffect:
	...
	def __init__(self, effect : effect, caster : entity, on : entity, icon : str, value=1, effReducPurcent:Union[None,int] = None, turnLeft:int = None):
		...
		self.effects: eff = copy.deepcopy(effect)
		...
		self.trigger = self.effects.trigger
		self.type = self.effects.type
		self.value = value
		...
		if self.effects.stat not in [None,FIXE,PURCENTAGE,MISSING_HP]:
			if self.effects.stat is not HARMONIE:
				statBucket = self.caster.allStats[self.effects.stat] + self.caster.actionStats[self.effects.actionStat]
			else:
				statBucket = max(self.caster.allStats) + max(self.caster.actionStats)

			if caster.aspiration in meleeAspi: statBucket += int(caster.endurance * MELEE_END_CONVERT/100)

			valueBoost = (min(self.caster.char.level,200)/50*0.3) + caster.valueBoost(self.on) * ((statBucket+100) /100)
			valueResis = caster.valueBoost(self.on) * (statBucket+100)/100
		...
		if self.effects.id in [vulne.id,dmgDown.id,dmgUp.id,defenseUp.id] and self.effects.stat not in [None,FIXE]:
			...
			self.effects.power = power * (100+maxStats-[caster.actHeal,caster.actShield][self.effects.id==convertEff.id])/100 * (1 + caster.valueBoost(self.on,heal=self.effects.id!=convertEff.id))/2

		self.caster.logs = self.logs
		try:
			if effect.stat not in [None,FIXE] and (not(tablTemp[0] == tablTemp[1] == tablTemp[2] == tablTemp[3] == tablTemp[4] == tablTemp[5] == tablTemp[6] == tablTemp[7] == tablTemp[8] == tablTemp[9] == 0) or effect.power != 0):
				...
				if effect.power != 0 and self.effects.id not in [vulne.id,dmgDown.id,dmgUp.id,defenseUp.id]:
					self.logs += "   Puissance : {0} [BaseValue : {1} * {2}] ;".format(self.effects.power,basePower,effReducPurcent/100)
				elif effect.power != 0 and self.effects.id in [vulne.id,dmgDown.id,dmgUp.id,defenseUp.id]:
					self.logs += "   Puissance : {0} [BaseValue : {1} * {2}] ;".format(self.effects.power,basePower,valueBoost*effReducPurcent/100)
				...
				if not(tablTemp[0] == tablTemp[1] == tablTemp[2] == tablTemp[3] == tablTemp[4] == tablTemp[5] == tablTemp[6] == tablTemp[7] == tablTemp[8] == tablTemp[9] == 0):
					...
					if effect.stat != PURCENTAGE:
						try:
							...
							temp = 0
						tempStats, tablVal, toAdd = temp+100-self.caster.actBoost, [min(self.caster.char.level,200)/50*0.3,caster.valueBoost(self.on),effReducPurcent/100], ""
						for cmpt in range(len(tablMul)):
							if tablVal[cmpt] != 1:
								toAdd += " * {0}".format(tablMul[cmpt].format(round(tablVal[cmpt],2)))
					else:
						tempStats, toAdd = 100, ""
					self.caster.logs += "   EffBoostPower : x{0} ({1} [stats+100] /100".format(valueBoost*effReducPurcent/100,tempStats) + toAdd + ") ;"
					tempLogs, baseStats = "", effect.allStats + [effect.resistance, effect.percing, effect.critical]
			
					for cmpt in range(CRITICAL+1):
						if tablTemp[cmpt] != 0:
							self.caster.logs += " {0} : {1} [BaseValue : {2}] ;".format(allStatsNames[cmpt],round(tablTemp[cmpt],2),baseStats[cmpt])
				self.caster.logs += "]\n"
		except:
			print_exc()
			pass
	...
	def triggerDamage(self, value=0, icon="", declancher = 0, onArmor = 1, armorSteal = 0, dealsDamageOn=False) -> list:
		...
		if self.effects.overhealth > 0 and value > 0 and self.value > 0:
			...
			if dmgRecived >= shieldPAr:
				...
				addedReduc = [self.on.char.level//[2,4][self.caster.char.__class__ == octarien],0][self.effects.lightShield]
	...
	def triggerDeath(self,killer=None) -> str:
		...
		if self.type==TYPE_INDIRECT_REZ:
		...
		elif self.type == TYPE_DEPL and type(self.effects.callOnTrigger) == depl:
			...
			if self.on.cell.on != None: toReturn += " ({0})\n".format(self.on.cell.on.icon)
			else: toReturn += "\n"

		if self.effects.callOnTrigger != None and type(self.effects.callOnTrigger) in [str,effect]:
	...
	def triggerStartOfTurn(self, decate=True) -> str:
		"""Déclanche l'effet"""
		funnyTempVarName=""
		if self.type == TYPE_INDIRECT_HEAL and self.on.hp > 0:			  # Heal Indirect
			...
			if self.effects.id != sixtineCardEff.id:
				...
				if drawedColor == 0:
					if drawedValue < 5 or drawedValue == 8:
						cardEff = effect(cardName,cardName,MAGIE,trigger=TRIGGER_END_OF_TURN,type=TYPE_INDIRECT_DAMAGE,power=([drawedValue+6,15][drawedValue==8])*10,emoji=cardEmoji,area=AREA_RANDOMENNEMI_1)
					else:
						cardEff = effect(cardName,cardName,MAGIE,trigger=TRIGGER_END_OF_TURN,type=TYPE_INDIRECT_DAMAGE,power={5:100,6:100,7:100}[drawedValue],emoji=cardEmoji,area={5:AREA_RANDOMENNEMI_3,6:AREA_RANDOMENNEMI_5,7:AREA_ALL_ENEMIES}[drawedValue])

				elif drawedColor == 1:
					if drawedValue < 5 or drawedValue == 8:
						triggerCardEff = effect(cardName+" (effet)",cardName,INTELLIGENCE,type=TYPE_MALUS,strength=([drawedValue+6,15][drawedValue==8])*-1,magie=([drawedValue+6,15][drawedValue==8])*-1,charisma=([drawedValue+6,15][drawedValue==8]//2)*-1,intelligence=([drawedValue+6,15][drawedValue==8]//2)*-1,emoji=':clubs:',turnInit=3)
						cardEff = effect(cardName,cardName,trigger=TRIGGER_END_OF_TURN,emoji=cardEmoji,area=[AREA_RANDOMENNEMI_1,AREA_RANDOMENNEMI_5][drawedValue == 8],callOnTrigger=triggerCardEff)
						...
						cardEff = effect(cardName,cardName,trigger=TRIGGER_END_OF_TURN,emoji=cardEmoji,area=AREA_ALL_ENEMIES,callOnTrigger=triggerCardEff)

				elif drawedColor == 2:
					if drawedValue < 5 or drawedValue == 8:
						cardEff = effect(cardName,cardName,MAGIE,trigger=TRIGGER_END_OF_TURN,type=TYPE_INDIRECT_HEAL,power=([drawedValue+6,10][drawedValue == 8])*10,emoji=cardEmoji,area=AREA_LOWEST_HP_ALLIE)
					elif drawedValue == 5:
						cardEff = effect(cardName,cardName,MAGIE,trigger=TRIGGER_END_OF_TURN,type=TYPE_INDIRECT_HEAL,power=100,emoji=cardEmoji,area=AREA_CIRCLE_2)
					elif drawedValue == 6:
						triggerCardEff = effect("Armure royale","royalarmor",PURCENTAGE,overhealth=50,turnInit=3,type=TYPE_ARMOR,trigger=TRIGGER_DAMAGE)
						cardEff = effect(cardName,cardName,trigger=TRIGGER_END_OF_TURN,type=TYPE_BOOST,emoji=cardEmoji,area=AREA_ALL_ALLIES,callOnTrigger=triggerCardEff)
					else:
						triggerCardEff = effect("Régénération royale","royalregen",PURCENTAGE,overhealth=12.5,turnInit=3,type=TYPE_INDIRECT_HEAL,trigger=TRIGGER_START_OF_TURN)
						cardEff = effect(cardName,cardName,trigger=TRIGGER_END_OF_TURN,type=TYPE_BOOST,emoji=cardEmoji,area=AREA_ALL_ALLIES,callOnTrigger=triggerCardEff)

				elif drawedColor == 3:
					if drawedValue < 5 or drawedValue == 8:
						triggerCardEff = effect(cardName+" (effet)",cardName,INTELLIGENCE,strength=[drawedValue+6,15][drawedValue == 8],magie=[drawedValue+6,15][drawedValue == 8],charisma=[drawedValue+6,15][drawedValue == 8]//2,intelligence=[drawedValue+6,15][drawedValue == 8]//2,emoji=':diamonds:',turnInit=3)
						cardEff = effect(cardName,cardName,trigger=TRIGGER_END_OF_TURN,emoji=cardEmoji,area=[AREA_RANDOMALLIE_1,AREA_ALL_ALLIES][drawedValue == 8],callOnTrigger=triggerCardEff)
						...
						valetEff.turnInit = queenEff.turnInit = kingEff.turnInit = 3
						triggerCardEff = {5:valetEff,6:queenEff,7:kingEff}[drawedValue]
						cardEff = effect(cardName,cardName,trigger=TRIGGER_END_OF_TURN,emoji=cardEmoji,area=AREA_ALL_ALLIES,callOnTrigger=triggerCardEff)
		...
		elif self.type == TYPE_SUMMON and type(self.effects.callOnTrigger) == invoc:
			...
			while cmpt < self.effects.lvl:
				...
				if cellToSummon != None:
					tempBlabla = self.caster.summon(toSummon, cellToSummon)
					funnyTempVarName += tempBlabla
					self.caster.logs += "\n"+tempBlabla
				cmpt += 1
		return funnyTempVarName

	def triggerEndOfTurn(self, decate=True) -> str:
		"""Trigger the effect"""
		temp = ""
		...
		if self.type == TYPE_INDIRECT_HEAL and self.on.hp > 0:			# End of turn healing (aka Light Aura 1)
			if self.effects.id == radianceEff.id:
				try:
					if self.on.otherVars["teamJaugeDict"][self.caster.team][self.caster].effects.id == healerGlare.jaugeEff.id and self.on.otherVars["teamJaugeDict"][self.caster.team][self.caster].value > 20:
						self.on.otherVars["teamJaugeDict"][self.caster.team][self.caster].value -= 20
						return self.triggeredIndHeal()
				except KeyError:
					pass
			else:
	...
	def triggerRemove(self) -> str:
		...
		if self.type == TYPE_UNIQUE:
			if self.effects.id == lostSoul.id:								   # Remove the body effect
				...
				giantessEff = effect("Puissance originelle","giantessNacialisla",
				...
				for key in naciaSplashIconPulls:
					if self.on.icon == naciaSplashIconPulls[key][0]: self.on.char.splashIcon = naciaSplashIconPulls[key][1]

				self.on.icon = self.on.char.splashIcon
				self.on.char.trait = self.on.char.trait + [TRAIT_GIANTESS]
				self.on.otherVars["naciaFlag"] = True
				add_effect(caster=self.on, target=self.on, effect=giantessEff)
	...
	def triggerInstant(self) -> str:
		...
		if self.effects.id == galvaniseEff.id: self.on.leftTurnAlive += self.effects.power
			...
			for eff in self.on.effects:
				if eff.effects.id == estial.id:
					...
					eff.decate(turn=99)
			if hasEstial:
				eff1 = effect("Explosion Féérique","fairyBoom",MAGIE,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_INSTANT,emoji=estial.emoji[0][0],power=int(sumPower*(FAIRYRAYMONODMG/100)))
				eff2 = effect("Explosion Féérique","fairyBoom2",MAGIE,type=TYPE_INDIRECT_DAMAGE,trigger=TRIGGER_INSTANT,emoji=estial.emoji[0][0],power=int(sumPower*(FAIRYRAYAOEDMG/100)),area=fairyRayEff.area)
			...
			for eff in self.on.effects:
				if eff.effects.id == sixtineCardEff.id:
					funnyTempVarName = eff.triggerStartOfTurn()
					break
			...
			if cumulatePower > 0:
				...
				toxiboomEff = effect("Marque d'Intoxication","marqueIntoxication",MAGIE,power=cumulatePower,turnInit=3,emoji='<:megaBact:1128350289980825601>',trigger=TRIGGER_END_OF_TURN,type=TYPE_INDIRECT_DAMAGE)
			...
			for eff in self.on.effects:
				...
				if eff.effects.id in [bleeding.id, kliBloodyEff3.id, coroWind.id]: tempEff = effect(eff.effects.name, eff.effects.id, eff.effects.stat, type=TYPE_INDIRECT_DAMAGE, power=eff.effects.power*eff.value, emoji=eff.icon, lifeSteal=eff.effects.lifeSteal, trigger=TRIGGER_INSTANT)
				elif eff.effects.id == deepWound.id: tempEff = effect(eff.effects.name, eff.effects.id+"1", eff.effects.stat, type=TYPE_INDIRECT_DAMAGE, power=eff.value, emoji=eff.icon, lifeSteal=50, trigger=TRIGGER_INSTANT)
		...
		if self.type == TYPE_DEPL and type(self.effects.callOnTrigger) == depl:
			self.caster.smnDepl(self.effects.callOnTrigger,self.on.cell)
			self.caster.logs += "\n{0} is placing a {1} at {2}:{3}".format(self.caster.name,self.effects.callOnTrigger.name,self.on.cell.x,self.on.cell.y)
			...
			if self.on.cell.on != None: funnyTempVarName += " ({0})\n".format(self.on.cell.on.icon)
			else: funnyTempVarName += "\n"
			...
			while cmpt < self.effects.lvl:
				...
				if cellToSummon != None:
					...
					tmpEff = effect("Explosion",toSummon.skills[0].id,toSummon.skills[0].use,area=toSummon.skills[0].area,type=TYPE_DAMAGE,trigger=TRIGGER_INSTANT,power=toSummon.skills[0].power,emoji=toSummon.skills[0].emoji)
					funnyTempVarName += groupAddEffect(caster=self.caster, target=self.on, area=self.on, effect=tmpEff)
				cmpt += 1

		if self.effects.callOnTrigger != None and type(self.effects.callOnTrigger) not in [invoc, depl]:
		...
		try: 
			for tmpConds in self.on.otherVars["teamJaugeDict"][self.caster.team][self.caster].effects.jaugeValue.conds:
				if tmpConds.type == INC_EFFECT_TRIGGERED:
					for tmpEff in tmpConds.add:
						if self.effects.id == tmpEff.id: self.on.otherVars["teamJaugeDict"][self.caster.team][self.caster].value = min(100, self.on.otherVars["teamJaugeDict"][self.caster.team][self.caster].value+tmpConds.value); funnyTempVarName += "{0} {1} → {0} {2} +{3}\n".format(self.caster.icon, self.icon, self.on.otherVars["teamJaugeDict"][self.caster.team][self.caster].icon,tmpConds.value)
		except KeyError: pass
	...
	def triggeredIndDamage(self,target,decate=True):
		...
		if self.effects.type == TYPE_INDIRECT_DAMAGE:
			...
			for secTarget in tablTarget:
				if secTarget != None:
					if secTarget.isNpc(TGESL1.name): secTarget = self.caster.entDict[secTarget.summoner.id]
					...
					tempToReturnDmg, tempLogs = indirectDmgCalculator(self.caster, secTarget, self.effects.power*reduc, self.effects.stat, area=self.effects.area, useActionStat=self.effects.actionStat)
					...
					if secTarget.hp <= 0 and secTarget.char.__class__ not in [invoc]:
						tempNbDeath += 1
					...
					if lifeStealValue and self.caster.id != target.id:
						if self.caster.hp < self.caster.maxHp or getChip("Sur-Vie").id in self.caster.char.equippedChips:
							...
							ballerine, chocolatine = self.caster.heal(target=[self.caster,self.on][self.effects.lifeStealOnOn], icon=self.icon, statUse=None, power=healPowa, effName = self.effects.name, lifeSteal = True,direct=False)
							self.caster.logs += ballerine
			...
			if self.caster.weaponEffect([lieWeap, kitsuneWeap]) or (self.caster.isNpc("Kitsune") and type(self.on.char != invoc) and ((skillToUse != None and skillToUse.id != kitsuneSkill4.id) or (skillToUse == None))):
	...
	def triggeredIndHeal(self, decate=True):
		...
		if self.effects.power > 0 or self.effects.stat in [FIXE, None, PURCENTAGE, MISSING_HP] and tablTargets != []:
			...
			for target in tablTargets:
				targetHp = target.hp
				funnyTempVarName, healed = self.caster.heal(target, self.icon, self.effects.stat, [self.effects.power,self.value][self.effects.stat in [None,FIXE]], self.effects.name, direct=False, incHealJauge = self.effects.incHealJauge, lifeSteal=self.effects.lifeSteal, useActionStats=self.effects.actionStat)
				if healed > 0:
	...
	def replaceEffect(self, effect: effect):
		...
		if self.effects.stat not in [None,FIXE,PURCENTAGE,MISSING_HP]:
			...
			valueBoost, valueResis = min(self.caster.char.level,200)/100 + self.caster.valueBoost(self.on) * (temp+100-self.caster.actBoost)/100, self.caster.valueBoost(self.on) * (temp+100-self.caster.actShield)/100
...
def add_effect(caster: entity, target: entity, effect: effect, start: str = "", ignoreEndurance=False, setReplica : Union[None,entity] = None, effPowerPurcent=100, useActionStats = ACT_SHIELD, skillIcon="", setValue=None):
	"""Méthode qui rajoute l'effet Effet à l'entity Target lancé par Caster dans la liste d'attente des effets"""
	if type(target) == entity and not(type(target.char) == depl) and type(effect) == effect:
		valid, popipo, friableValue = False,"", 0
		valid = True
		...
		if effect.id == elemEff.id:
			...
			for eff in target.effects:
				if eff.effects.id == effect.id and eff.caster.id == caster.id:
					...
					ballerine, babies = eff.caster.heal(target=caster, icon=eff.icon, statUse=PURCENTAGE, power=eff.effects.power, effName = eff.name, mono = True, useActionStats = ACT_HEAL, lifeSteal = False,direct=False)
					eff.decate(turn=99)
					popipo += ballerine
					break
			...
			value, boostedValue, resistedValue = dmgCalculator(caster=caster, target=target, basePower=(effect.power*effPowerPurcent/100), use=effect.stat, actionStat=ACT_DIRECT, area=AREA_MONO)
			...
			potShieldValue, critMsg = calShieldValue(caster,target,(effect.overhealth*effPowerPurcent/100),effect.stat, turn, actStat=useActionStats)
			for eff in target.effects:
				if eff.effects.id == lavenderShield.id:
					ballerine, babies = caster.heal(target, icon=effect.emoji[0][0], statUse=None, power=[eff.value,potShieldValue][eff.value >= potShieldValue], effName = effect.name, direct=True)
					...
					if eff.value < potShieldValue:
						...
						target.effectWaitList.append(fightEffect(id,effect,caster,target,effect.turnInit,effect.trigger,TYPE_ARMOR,eff.icon,potShieldValue))
						target.refreshEffects()
		...
		if valid:
			...
			if effect.id == darkFlumPoi.id: effect = darkFlumPoi; effect.power = int(caster.char.level * 0.2)
				...
				if effect.stat in [None,FIXE]:
					...
					boostHp = effect.power * (100+(caster.allStats[effect.stat]+[caster.actHeal,caster.actBoost,caster.actShield,caster.actDirect,caster.actIndirect][useActionStats]))/100
				...
				if not(effect.turnInit < 0 and effect.unclearable):
					target.effectWaitList.append(toAppend)
					target.refreshEffects()
				...
				actPVPurcent = target.hp / target.maxHp
				if effect.stat in [None,FIXE]: boostHp = effect.power
				elif effect.stat == PURCENTAGE: boostHp = int(target.maxHp*(effect.power/100))
				else:
					if useActionStats == None: useActionStats = ACT_BOOST
					boostHp = int(effect.power * (100+(caster.allStats[effect.stat]+[caster.actHeal,caster.actBoost,caster.actShield,caster.actDirect,caster.actIndirect][useActionStats]))/100)
			...
			if effect.overhealth > 0:									   # Armor Effect
				...
				value, critMsg = calShieldValue(caster, target, shieldPower, effect.stat, turn, actStat=useActionStats)
				...
				if value>0:
					...
					target.effectWaitList.append(toAppend)
					if not(effect.silent): popipo = f"{caster.icon} {skillIcon} → {target.icon} {icon} +{separeUnit(value)} PAr{critMsg}\n"
					...
					if effect.id in [estial.id,bleeding.id]:							   # If is the Estialba effect
						...
						for eff in caster.effects:
							if eff.effects.id == effLookingFor.id:
								...
								icon = effect.emoji[caster.char.species-1][caster.team]
								target.effectWaitList.append(fightEffect(1,effect,caster,target,effect.turnInit,effect.trigger,effect.type,icon,effect.lvl))
						...
						if len(tempTabl) > 0:
							...
							target.effectWaitList.append(fightEffect(id,effect,caster,tempTabl[0],effect.turnInit,effect.trigger,effect.type,icon,effect.lvl))
							tempTabl[0].refreshEffects()
							toAppend.replicaTarget = tempTabl[0]

				else: popipo = toAppend.triggerInstant()

		if target.hp > 0 or effect.trigger == TRIGGER_INSTANT: return popipo
		else: return ''
	else: return ""

def groupAddEffect(caster:entity, target:entity, area:Union[int,List[entity]], effect:List[effect], skillIcon="", actionStats=ACT_BOOST,effPurcent=100):
...
def addingEffect(caster:entity, target:entity, area:Union[int,List[entity]], effect:List[effect], skillIcon="", actionStats=ACT_BOOST,effPurcent=100):
	toReturn = ""
	if type(effect) != list: effect = [effect]

	for c in effect:
		eff:effect = findEffect(c)
		if eff != None:
