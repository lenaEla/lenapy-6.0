...
def getAllieFromEnemy(enemy:octarien,lvl:int,gearEmotes:List[str]=[None,None,None],color=None) -> tmpAllie:
	...
	toReturn.changeLevel(lvl,False,0)
...
def aformRandomiser(entity: octarien, nb: int = None):
	if entity.name in dictAform.keys():
		entity = copy.deepcopy(entity)
		if nb == None: newData = randRep(dictAform[entity.name])
		else: newData = dictAform[entity.name][nb % len(dictAform[entity.name])]
		entity.name = newData[0]
		if newData[1] != "": entity.icon = newData[1]
		entity.weapon = newData[2]
		entity.skills = newData[3]
		entity.strength, entity.endurance, entity.charisma, entity.agility, entity.precision, entity.intelligence, entity.magie, entity.resistance, entity.percing, entity.critical = newData[4][0], newData[4][1], newData[4][2], newData[4][3], newData[4][4], newData[4][5], newData[4][6], newData[4][7], newData[4][8], newData[4][9]
		entity.aspiration = newData[5]
	return entity